...
![채널을 연결하기 전 Reviewnary 첫 화면 — 연결 후 무엇을 대신 확인하는지 세 단계로 안내한다](docs/images/readme/01-home-before-connect.png)

*연결 전 첫 화면. 아직 아무 데이터도 없을 때 0으로 채운 대시보드 대신, 무엇을 대신 해 주겠다는
것인지와 다음 한 걸음만 말합니다.*
...
![확인할 일 화면 — 왼쪽은 채널을 가로지른 대기 목록, 오른쪽은 선택한 리뷰의 상세](docs/images/readme/03-queue-with-detail.png)

*Operations Case — 채널마다 따로 열어 보던 것이 한 목록이 됩니다. 정렬 기준은 고객이 기다린
시간이고, 행을 고르면 그 건의 판단 화면이 옆에 섭니다.*

![리뷰 판단 화면 — 중요도와 처리 방법 두 축, 그리고 마켓플레이스 전송 없음 안내](docs/images/readme/04-review-case.png)

*Seller Decision — 판매자가 내리는 판단은 두 가지입니다. 이 건이 지금 볼 일인가, 그리고 무엇을 할
것인가. 답글 경로가 없는 채널에서는 그 사실을 말하고, 기록만 남깁니다.*

마지막 단계가 핵심입니다. 실행의 성공 판정은 **2xx가 아니라** 채널을 다시 읽어 게시된 글이 실제로
존재하고 승인된 초안과 같은지 확인한 결과입니다.

> 위 세 화면은 저장소의 로컬 데모 fixture(`MockDataSeeder`)로 렌더한 것입니다. 실제 판매자 · 고객
> 데이터는 들어 있지 않습니다.
