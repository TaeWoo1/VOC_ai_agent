...
/**
 * Production {@link ElevenstHttpClient} over the JDK HTTP client. Created only
 * by {@link ElevenstConnectorConfiguration}, i.e. only when the 11st feature
 * flag is on — tests and the default runtime never instantiate it. Failure
 * messages carry no request material (the headers contain the static
 * {@code openapikey}).
 */
public class JdkElevenstHttpClient implements ElevenstHttpClient {
    ...
    private Response send(HttpRequest request) {
        try {
            ...
            throw new IllegalStateException("11번가 API 호출에 실패했습니다 (네트워크 오류).");
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new IllegalStateException("11번가 API 호출이 중단되었습니다.");
        }
    }
