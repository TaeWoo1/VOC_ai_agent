 ...
 * happened (in Phase 3D-6 nothing should ever be enqueued).
 */
final class FakeSsgHttpClient implements SsgHttpClient {

    record Sent(String method, URI uri, Map<String, String> headers) {

        /** Failed-assertion output must not echo the static vendor key. */
        @Override
            ...
            if (headers != null) {
                ...
                for (Map.Entry<String, String> entry : headers.entrySet()) {
                    masked.put(entry.getKey(),
                            SsgApiConnector.AUTH_HEADER.equalsIgnoreCase(entry.getKey())
