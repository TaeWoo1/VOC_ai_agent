...
echo "    • It asks the page whether anything carries an identifier SellerOps ALREADY HOLDS — the identifiers"
echo "      go IN, and a count comes back. 0 and 2 are both refusals; only exactly 1 is a target."
echo "    • It looks in TWO places and no others: link addresses / element ids / data-* attributes, and the"
echo "      cells of the ONE column headed 문의유형(접수번호)."
echo "    • The 고객명, 상품명 and 문의내용 columns are NOT compared against anything. The column scope is"
echo "      also what stops an inquiry number from being matched against a 주문번호 in the next column."
echo "    • Around a single match it then measures SHAPE: what tag it is, how many identically shaped"
echo "      siblings the thing above it has, whether that row offers a way into a detail view, and whether"
echo "      it says 답변완료."
echo "    • **What your customers wrote never leaves this window.** The page's text is compared, inside the"
echo "      page, against strings SellerOps supplied — its own inquiry numbers, and fixed Coupang words like"
echo "      답변완료 — and only the COUNT comes out. No customer text, no link address, no class name is"
