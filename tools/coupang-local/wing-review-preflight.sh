...
echo "    • It has NO review number to look for. Coupang publishes no review API, so SellerOps holds no review"
echo "      id, date or rating — this screen is first contact, and the run measures WHAT IT IS."
echo "    • The one question it exists to answer: **is there a 판매자 답글 control on this screen?** A 답글"
echo "      button is a capability; 답글여부 printed in a header is a word. It counts them separately and"
echo "      presses neither."
echo "    • It then measures SHAPE: which repeating unit Coupang's own words (평점 · 작성일 · 상품평 …) agree"
echo "      on, how many of those units carry a photo or a video, whether a rating widget is present, and how"
echo "      long the numbers in each unit are."
...
echo "      read. Dates come back as WHICH FORMAT matched and how many, never as a date."
...
echo "    • Why this exists: whether Coupang review operations can ever reach a guided reply depends entirely"
echo "      on whether that control exists, and building either answer before measuring would be a guess."
echo
