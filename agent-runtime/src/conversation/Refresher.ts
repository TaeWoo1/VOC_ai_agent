...
export class Refresher implements ReviewRefresher {
  ...
  async refresh(accountId: string, dataType: "REVIEW"): Promise<RefreshOutcome> {
    ...
    try {
      ...
      if (status === "RUNNING") {
        // Single-flight coalesced this call onto a run already in progress: the rows are not yet current.
      ...
      log("conversation_refresh", { dataType, ok: true, status, successRows: run.successRows ?? null, ms: Date.now() - started });
      return { ok: true, finishedAt, successRows: run.successRows ?? null, status };
    } catch (err) {
