 ...
 * inquiry approve loop ({@link ./inquiryGraph}), this subgraph reads the OPEN queue and one item's
 * detail and then asks the product's own draft path for a draft ({@link ComposerDraftProvider} →
 * `InquiryDraftComposer`: propose when OPEN, append one MODEL version). It never records an approval
 * and never moves anything toward a channel — the "Human Checkpoint" here is terminal presentation:
 * the run finishes with the draft in hand and hands off to the human, who edits/copies it and posts
 * on the channel manually, or opens the inquiry screen where the same saved version waits. So the run
 * goes straight through to DONE, and the caller ({@link ../inquiryDraftRuntime}) projects the final
 * state into a sanitized draft view.
 *
 * The graph's TOOL registry is the READ-ONLY inquiry registry (search + detail only): no propose/save/
 * record tool exists on this path, no `interrupt`, no resume. The one PREPARE — the draft version the
 * composer saves — goes through the drafter seam, not a tool, and is the same append-only write the
 * inquiry screen makes (Knowledge Context v1-A closure, 2026-08-30). Ranking is the shared prioritizer;
 * the state shape is {@link AgentStateAnnotation} with `decision`/`outcome` never written.
 *
 * Determinism: for a fixed backend state the search order and the selection are deterministic; the
 * draft TEXT is the backend's and is re-read from the saved version, not regenerated by this graph.
 * (The generation timestamp is stamped by the runtime, not here, so this graph stays clock-free.)
 */
