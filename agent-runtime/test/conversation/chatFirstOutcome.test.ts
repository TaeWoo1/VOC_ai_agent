...
describe("§3 — the freshness question is a shape, not a sentence", () => {
  it("takes the three-part question and refuses everything wider", () => {
    expect(freshnessQuestionOf("오늘 네이버 리뷰 있어?")).toEqual({ period: "TODAY" });
    expect(freshnessQuestionOf("리뷰 뭐 들어왔어?")).toEqual({ period: null });
    expect(freshnessQuestionOf("이번 달 리뷰 어때?")).toEqual({ period: "THIS_MONTH" });
  ...
  it("a planner that cannot plan it does not end the turn — the product answers from its own reads", async () => {
    const h = harness();
    ...
    // No recorded plan for this sentence: the planner produces nothing, exactly as it did live.
    ...
    expect(turn.budget?.stopReason).toBe("FRESHNESS_RECOVERED");
  });
