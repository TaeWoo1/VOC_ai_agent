...
const REASON_COPY: Record<WalkthroughMismatchReason | "HANDSHAKE_FAILED", string> = {
  ...
  MISSING_CONTEXT: "연결된 서버가 walkthrough 실행 정보를 제공하지 않습니다. 승인된 서버가 아닐 수 있습니다.",
  RUN_MISMATCH: "주소·프론트엔드·서버의 walkthrough 실행 ID가 서로 다릅니다. 다른 실행이나 오래된 탭일 수 있습니다.",
  ORIGIN_MISMATCH: "이 탭의 origin이 승인된 origin과 다릅니다. (예: 127.0.0.1 대신 localhost)",
  HANDSHAKE_FAILED: "서버 핸드셰이크가 이 실행/origin과 일치하지 않습니다. 승인된 URL로 다시 열어 주세요.",
};
  ...
  return (
    <section className="card space-y-4 border-bad p-6" role="alert" aria-label="WALKTHROUGH_ENVIRONMENT_MISMATCH">
      ...
      <p className="text-sm text-muted">
        이 브라우저 탭이 승인된 walkthrough 환경(같은 프론트엔드·서버·DB·실행)에 연결되어 있는지 확인하지 못했습니다.
        안전을 위해 연결 정보 입력과 연결 테스트·수집을 모두 막았습니다. 자동으로 다른 서버를 찾지 않습니다.
      </p>
