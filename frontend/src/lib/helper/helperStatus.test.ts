...
describe("helperStatusOf — six words, one action each, no internal concept", () => {
  ...
  it("a running helper this browser is not connected to is 다시 연결 필요 — with the reason when there is one", () => {
    expect(helperStatusOf({ phase: "unpaired", pairedBefore: false, agentVersion: "0.1.0" })).toMatchObject({ key: "RECONNECT", action: { kind: "connect" } });
    expect(helperStatusOf({ phase: "revoked", pairedBefore: true, agentVersion: "0.1.0" }).note).toContain("해제");
    expect(helperStatusOf({ phase: "pairing_denied", pairedBefore: true, agentVersion: "0.1.0" }).note).toContain("거부");
    expect(helperStatusOf({ phase: "unpaired", pairedBefore: true, agentVersion: "0.1.0", pairingHint: "no_response" }).note).toContain("응답이 없어");
  });
  ...
  it("an old helper is 업데이트 필요 whether the wire refused it or the version is simply below the minimum", () => {
    ...
    expect(helperStatusOf({ phase: "paired", pairedBefore: true, agentVersion: "0.1.0" }).key).toBe("CONNECTED");
  ...
  it("a pending approval says where to look, and never shows a code word", () => {
    const s = helperStatusOf({ phase: "pairing_pending", pairedBefore: false, agentVersion: "0.1.0", attestedApproval: true });
  ...
  it("every note and label is free of internal words", () => {
    for (const phase of ["connecting", "unreachable", "unpaired", "pairing_pending", "pairing_denied", "paired", "incompatible_version", "disconnected", "revoked"]) {
      for (const pairedBefore of [true, false]) {
        const s = helperStatusOf({ phase, pairedBefore, agentVersion: "0.1.0", maybeNeedsLocalNetworkAccess: true });
