...
export type CoupangPhase =
  ...
  | "issuance" // steps 1–2: agent-driven WING Open API key issuance walkthrough (or text fallback)
...
export type ResolvedPhase = Extract<
  CoupangPhase,
  "issuance" | "connect" | "connect_error" | "preparing" | "syncing" | "sync_error" | "connected" | "unavailable"
>;
...
export type CoupangEvent =
  ...
  | { type: "ISSUANCE_DONE" } // WING issuance complete OR seller has the key already (issuance → connect)
...
export function coupangTutorialReducer(state: CoupangState, event: CoupangEvent): CoupangState {
  switch (event.type) {
    ...
    case "ISSUANCE_DONE":
      // Issuance is a pre-credential stage; completing it (or skipping via "I already have the key")
      // hands off to credential entry. Only meaningful from `issuance` — ignored elsewhere (fail-safe).
      if (state.phase !== "issuance") return state;
...
/**
 * Derive the resume phase purely from persisted state — the heart of refresh/leave recovery. A brand-new
 * seller (no account, no credential) enters the WING key-issuance walkthrough (`issuance`). Any seller who
 * has already progressed SKIPS issuance: a credential on file but not yet PREPARING → recovery (unverified,
 * reason unknown after a reload); PREPARING → observe a running sync, retry a failed one, else offer the
 * first-sync CTA; CONNECTED → done. So issuance is reached ONLY at a truly fresh start, and a refresh during
 * issuance (nothing persisted yet) re-lands on it — the hosted issuance run reattaches idempotently.
 */
export function resolvePhase(input: CoupangResolveInput): ResolvedPhase {
  ...
  // on recovery so the seller can re-verify or re-enter, without inventing a past error reason. This also
  // SKIPS issuance for an already-issued seller (they have a stored credential).
  if (input.credentialPresent) return "connect_error";
  // A truly fresh seller: no account and no credential → start with WING key issuance.
  return "issuance";
}
...
export function activeStep(phase: CoupangPhase): number {
  switch (phase) {
    case "issuance":
      return 1; // the WING key-issuance walkthrough is step 1 (API 키 발급)
    case "resolving":
    case "connect":
    case "submitting":
    case "connect_error":
...
export const COUPANG_TUTORIAL_COPY = {
  ...
  step1Body:
    "쿠팡 윙 › 판매자 정보 › 오픈API 키 발급에서 액세스 키·시크릿 키를 발급하세요. 발급 버튼을 누른 뒤 사용 목적 확인 → 약관 동의 → 업체 정보 입력을 거쳐, 마지막 '확인'에서 키가 만들어집니다.",
  step1SelfDev:
    "마지막 화면의 ‘업체 입력 방식’에서 ‘자체개발(직접입력)’을 선택하세요. 솔루션사(대행) 연동이 아니라 내 시스템에서 직접 호출하는 방식입니다. 첫 화면의 ‘사용 목적’에는 이 항목이 없으니 ‘OPEN API’(기본값) 그대로 두시면 됩니다.",
  ...
  step2CallIp: "호출 IP: 쿠팡은 등록된 IP에서만 API 요청을 허용합니다. 아래 IP를 앱의 호출 IP에 넣고 옆의 ‘추가’까지 누르세요 — 누르지 않으면 등록되지 않습니다.",
