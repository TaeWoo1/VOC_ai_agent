...
describe("coupangTutorialReducer", () => {
  ...
  it("RESOLVED can land a fresh seller on the issuance phase", () => {
    const s = coupangTutorialReducer(INITIAL_COUPANG_STATE, { type: "RESOLVED", phase: "issuance" });
    expect(s).toEqual({ phase: "issuance", reasonCode: null });
  });

  it("ISSUANCE_DONE advances issuance → connect (credential entry hand-off)", () => {
    const s = coupangTutorialReducer({ phase: "issuance", reasonCode: null }, { type: "ISSUANCE_DONE" });
    expect(s).toEqual({ phase: "connect", reasonCode: null });
  });

  it("ISSUANCE_DONE is a no-op outside issuance (pure, fail-safe)", () => {
    const connect: CoupangState = { phase: "connect", reasonCode: null };
    expect(coupangTutorialReducer(connect, { type: "ISSUANCE_DONE" })).toBe(connect);
    const connected: CoupangState = { phase: "connected", reasonCode: null };
    expect(coupangTutorialReducer(connected, { type: "ISSUANCE_DONE" })).toBe(connected);
  });

  it("SUBMIT is ignored from issuance — credential entry is only reachable via ISSUANCE_DONE first", () => {
    const issuance: CoupangState = { phase: "issuance", reasonCode: null };
    expect(coupangTutorialReducer(issuance, { type: "SUBMIT" })).toBe(issuance);
  });
...
describe("resolvePhase (refresh / leave recovery)", () => {
  ...
  it("fresh seller (no account, no credential) → issuance (WING key issuance walkthrough first)", () => {
    expect(resolvePhase({ ...base, connectionStatus: null })).toBe("issuance");
  });

  it("credential on file but not PREPARING → connect_error, SKIPPING issuance (already-issued)", () => {
    ...
    // An account with a credential never re-enters issuance.
    expect(resolvePhase({ ...base, connectionStatus: "PENDING", credentialPresent: true })).not.toBe("issuance");
  });

  it("already-progressed sellers skip issuance entirely (PREPARING / CONNECTED land past it)", () => {
    expect(resolvePhase({ ...base, connectionStatus: "PREPARING", credentialPresent: true })).not.toBe("issuance");
    expect(resolvePhase({ ...base, connectionStatus: "CONNECTED", credentialPresent: true })).not.toBe("issuance");
  });
...
describe("stepModel / activeStep", () => {
  it("advances the active step across the journey", () => {
    expect(activeStep("issuance")).toBe(1);
  ...
  it("issuance marks step 1 current, everything after upcoming", () => {
    const m = stepModel("issuance"); // active 1
    expect(m[0].state).toBe("current"); // step 1 (API 키 발급)
    expect(m.slice(1).every((s) => s.state === "upcoming")).toBe(true);
  });
