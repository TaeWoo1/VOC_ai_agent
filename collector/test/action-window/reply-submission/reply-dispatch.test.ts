...
describe("recoverReplyRuns — restart PARKS interrupted runs, never resumes", () => {
  it("marks a non-terminal run PARKED and leaves terminal runs untouched; idempotent", () => {
    ...
    saveReplyRun(dir, { schemaVersion: 1, runId: "run_aaaaaaaaaaaa", channelCode: "naver", stage: "WAIT_FOR_SUBMIT", parked: false, updatedAt: now() });
    saveReplyRun(dir, { schemaVersion: 1, runId: "run_bbbbbbbbbbbb", channelCode: "naver", stage: "OPERATOR_REPORTED", parked: false, updatedAt: now() });
...
describe(".reply-runs store — sanitized, fail-closed", () => {
  it("round-trips a valid marker and lists it", () => {
    const dir = newDir();
    const rec = { schemaVersion: 1 as const, runId: "run_cccccccccccc", channelCode: "naver", stage: "PREPARE_SESSION" as const, parked: false, updatedAt: "reply-run.000001" };
  ...
  it("rejects a malformed record on load", () => {
    const dir = newDir();
    writeFileSync(join(dir, "run_dddddddddddd.json"), JSON.stringify({ schemaVersion: 1, runId: "run_dddddddddddd", channelCode: "naver", stage: "NOT_A_STAGE", parked: false, updatedAt: "x" }));
    expect(() => loadReplyRun(dir, "run_dddddddddddd")).toThrow(/STORE_INVALID_RECORD/);
  });

  it("refuses a runId that is not opaque-id-safe", () => {
    const dir = newDir();
    expect(() => saveReplyRun(dir, { schemaVersion: 1, runId: "../evil", channelCode: "naver", stage: "PREPARE_SESSION", parked: false, updatedAt: "x" })).toThrow();
