...
export function runApprovalManifestCli(opts: ApprovalManifestCliOptions = {}): number {
  ...
  const defaultOperation = isWingKeyDeletion
    ...
    : isWingVendorMethod
    ? `WING VENDOR-METHOD DISCOVERY across ${checkpoints.length} operator-advanced checkpoints (${checkpoints.join(" → ")}). The whole issuance flow as the discovery phase runs it, and then two steps further: the OPERATOR presses \`약관 동의 및 Key 발급받기\` themselves${reachesVendorScreen ? "" : " — NOT in this narrowed run, which ends earlier"}. That press was believed to create the key, was pressed on two live walks, and issued NO key either time (WING_KEY_CREATION_CONTROL_REFUTATION) — this phase rests on that measurement rather than on the button's label, which is what the refuted claim rested on. What it opens is an integration-method screen (\`업체 입력 방식\` / \`연동업체 선택\` / \`자체개발(직접입력)\` / \`업체명\` / \`취소\` \`확인\`) that NO apparatus has ever read; the agent sweeps it read-only${reachesVendorSelection ? " and the operator then selects an input method, which is where the run ENDS" : ""}. ⚠ THAT SCREEN'S \`확인\` ISSUES A REAL API KEY, irreversibly, on the operator's live account. It is not in this approval, no checkpoint of this phase stands in front of it, and the phase has no checkpoint after the last one above. Key issuance is a separate manifest and a separate mode-WRITE grant. WHICH input method SellerOps should use is a PRODUCT DECISION and is not answered by this run — the run measures what the screen is made of and recommends nothing. The agent performs no click, selection, input, or value read, and never reads \`checked\`.`
    : isWingGuidedWalk
    ? "WING GUIDED ISSUANCE WALK, end to end — and THIS RUN ENDS WITH A REAL API KEY ON YOUR LIVE COUPANG ACCOUNT. ⚠ That is new: every earlier walk stopped one screen short of any key existing. The OPERATOR performs every marketplace action: log in, reach the page, press 'API Key 발급 받기', confirm OPEN API is selected, press 확인, read the two consent texts and tick them, press '약관 동의 및 Key 발급받기' (MEASURED to create no key — pressed twice on live walks, none issued), select the input method, type their own 업체명 · URL · IP 주소, and press the vendor screen's '확인' — WHICH ISSUES THE KEY, IRREVERSIBLY. SellerOps never presses it, never types into any field, and has no code path that could. The agent OPENS the seller's own WING sales-info landing once, so the window is not blank, and navigates no further. It highlights NINE live-calibrated controls; every guided step that names a WING CONTROL rings it, and the two that name no control — reaching the page, and going back to SellerOps — are still text-only. The rings on the purpose option, the consents and the vendor method sit on the LABEL and the SENTENCES, never on a radio or a checkbox: those inputs have no accessible association, so SellerOps does not claim to know which is which. It ADVANCES ITSELF on WING's own state — the purpose screen appearing, the terms screen appearing, both consent boxes being ticked (a yes/no computed in the page, never stored, sent, or logged), the vendor screen appearing, and finally the credentials appearing. That last one is an observation of the RESULT and cannot cause it: the page category cannot become `credential_shown` before a credential exists. WHICH input method is guided ('자체개발(직접입력)') is a PRODUCT DECISION recorded as one, not a measurement. No credential value read, no connect-test, no sync, no upload."
    : isWingReveal
  ...
  const defaultMaxActions = isWingKeyDeletion
    ...
    ? "operator-performed: 1 발급 press + 1 purpose-option selection (none needed if OPEN API is already the default)" +
      ...
      (reachesTerms ? " + up to 2 consent checkbox ticks" : "") +
      (reachesVendorScreen ? " + 1 press of 약관 동의 및 Key 발급받기 (measured to issue no key: pressed twice live, none issued)" : "") +
