...
export function runApprovalManifestCli(opts: ApprovalManifestCliOptions = {}): number {
  ...
  // The checkpoint added on 2026-08-13, where the seller fills the form in. Its own flag, like every other step
  // here, so a NARROWED run that stops before it does not describe itself as reaching it.
  const reachesVendorForm = checkpoints.includes("VENDOR_FORM_IP_REGISTERED_BY_OPERATOR");
