...
export function runApprovalManifestCli(opts: ApprovalManifestCliOptions = {}): number {
  ...
  const defaultMaxActions = isWingKeyDeletion
    ...
    : isWingGuidedWalk
    ? "operator-performed: the whole tutorial through the consent step (1 발급 press + 1 확인 press on the merged purpose step + up to 2 consent ticks); 0 presses of the key-creating 약관 동의 및 Key 발급받기 button. agent: 2 highlights of live-calibrated controls, 3 text-guided steps with no highlight, 4 steps advanced by OBSERVING WING (the key-creating step never), 0 clicks, 0 inputs, 0 submits, 0 navigations, 0 credential-value reads"
    : isWingReveal
