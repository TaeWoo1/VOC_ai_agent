...
export function runApprovalManifestCli(opts: ApprovalManifestCliOptions = {}): number {
  ...
  const defaultOperation = isWingKeyDeletion
    ...
    : isWingGuidedWalk
    ? "WING GUIDED ISSUANCE WALK, end to end (the OPERATOR performs every marketplace action: log in, reach the page, press 'API Key 발급 받기', confirm OPEN API is selected, press 확인, read the two consent texts and tick them. The agent highlights the TWO live-calibrated controls, TEXT-GUIDES the four that were measured but never promoted — claiming no locator and drawing no ring at them — rests at every checkpoint, and clicks, types, submits and NAVIGATES nothing. THE WALK RESTS IN FRONT OF `약관 동의 및 Key 발급받기`, which is the control that CREATES THE KEY: it is never pressed, no step follows it here, and key issuance is a separate phase with its own manifest and grant. No credential value read, no connect-test, no sync, no upload)"
    : isWingReveal
  ...
  const defaultMaxActions = isWingKeyDeletion
    ...
    : isWingGuidedWalk
    ? "operator-performed: the whole tutorial through the consent step (1 발급 press + 1 purpose confirmation + 1 확인 press + up to 2 consent ticks); 0 presses of the key-creating 약관 동의 및 Key 발급받기 button. agent: 2 highlights of live-calibrated controls, 4 text-guided steps with no highlight, 0 clicks, 0 inputs, 0 submits, 0 navigations, 0 credential-value reads"
    : isWingReveal
