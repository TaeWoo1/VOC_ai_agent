/**
 * **Dependency-free, READ-ONLY structural xlsx reader — the pure, buffer-in core.**
 *
 * An xlsx is a ZIP of XML parts. This module walks the ZIP central directory, inflates ONLY the
 * parts needed for *shape* (`xl/workbook.xml`, the first worksheet, `xl/sharedStrings.xml`), and
 * scans them with bounded regex. It uses **`node:zlib` only** — no `node:fs`, no network, no
 * browser — so callers that are forbidden from touching the filesystem (the Action Window's
 * artifact parse gate) can reach it, while file-path callers add `readFileSync` themselves.
 *
 * This code was extracted verbatim from `../esm/esm-review-xlsx-reader.ts` (Gate 4/5), which now
 * delegates here and keeps its own file-path API. Behaviour is unchanged; the only difference is
 * where the bytes come from.
 *
 * ZIP64 / encrypted / unexpected containers fail closed (`workbookReadable:false` + a sanitized
 * reader-risk token) — never a throw to the caller.
 */
import { inflateRawSync } from "node:zlib";
import type { WorkbookShape } from "../esm/esm-review-schema-shape";
...
/** The fail-closed unreadable shape, carrying only sanitized risk tokens. */
export function unreadableShape(risks: string[]): WorkbookShape {
  return {
...
/**
 * Read xlsx BYTES into a sanitisable `WorkbookShape`. Never throws: a non-ZIP container, ZIP64, or
 * a corrupt part each yields `workbookReadable:false` + a sanitized reader-risk token. Reads no
 * data-row values.
 */
export function workbookShapeFromBuffer(buf: Buffer): WorkbookShape {
  if (buf.length < 4 || buf.readUInt32LE(0) !== LFH_SIG) return unreadableShape(["not-zip-container"]);

  const entries = readZipEntries(buf);
  if (entries === null) return unreadableShape(["zip-parse-failed-or-zip64"]);

  const workbookPart = entries.get("xl/workbook.xml");
  if (!workbookPart) return unreadableShape(["no-workbook-part"]);
 ...
 * and NEVER emitted. Reading them is the only difference from `workbookShapeFromBuffer`.
 */
...
/** Widen a shape into an empty row sample (no header cells, no data rows). */
export function emptySample(base: WorkbookShape): WorkbookRowSample {
  return { ...base, headerCells: [], sampleRows: [] };
}

/**
 * Read xlsx BYTES into a `WorkbookRowSample`: the same structural shape as
 * {@link workbookShapeFromBuffer} PLUS the first ≤`maxDataRows` populated data rows
 * (column-aligned, raw). Never throws — every failure path yields an unreadable shape with empty
 * samples + a sanitized risk.
 */
export function workbookRowSampleFromBuffer(buf: Buffer, maxDataRows: number): WorkbookRowSample {
  if (buf.length < 4 || buf.readUInt32LE(0) !== LFH_SIG) return emptySample(unreadableShape(["not-zip-container"]));

  const entries = readZipEntries(buf);
  if (entries === null) return emptySample(unreadableShape(["zip-parse-failed-or-zip64"]));

  const workbookPart = entries.get("xl/workbook.xml");
  if (!workbookPart) return emptySample(unreadableShape(["no-workbook-part"]));
  const sheetCount = countSheets(workbookPart.toString("utf8"));
