 ...
 *   - It NEVER uploads, writes status, mutates a DB, or sets LAST_SUCCESS. No xlsx parser is used —
 *     validation is a bounded magic-byte sniff (ZIP local-header + OOXML marker), so NO cell is ever
 *     read and `rawCellLeak` is structurally false.
...
export async function saveAndInspectDownload(
  ...
  try {
    ...
    return {
      ...
      xlsxReadable: sniffXlsxReadable(head),
      workbookContentValidation: "deferred",
      rawCellLeak: false,
      fileRetained: false,
      retentionPolicy: "delete-after-validate",
