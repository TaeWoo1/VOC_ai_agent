/**
 * **Official, reusable NAVER API-issuance guided-walk LIVE-PROOF driver (bridge client) — GATED, read-only.**
 *
 *   npx tsx instruments/live-runs/issuance-live-proof.ts -- --i-understand-this-opens-live-naver
 *
 * This is the committed replacement for the ad-hoc scratchpad `issuance-*-runner.mjs` clients used during the
...
import { hasLiveRunApproval, approvalRequiredMessage } from "../../src/cli/live-run-approval";
...
/** A single guided-walk session over the adopted issuance run. Read-only: it only sends the two guidance commands. */
class LiveProofSession {
