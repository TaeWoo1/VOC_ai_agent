...
- **ACTION_WINDOW** — SellerOps가 실제 마켓 페이지 + 튜토리얼 오버레이를 준비, 사용자가 실제 마켓에서
  ...
  production 설계이며 아직 구현·라이브 검증 안 됨** — 모드 배정은 목표 표현이고, 실제 운영 검증 상태는 표 B의
  `구현`·`검증` 열(§4.1 인용)을 본다. 셀러에게 "이미 제공"으로 표기 금지.
...
| NAVER | 리뷰 | ✅ collector | ⚠ 캡처→저장 1회; 자동 업로드 미검증 | ⚠ 셀러-통제 로컬, 마켓 약관 해명 필요 | — | Action Window 보정(별도 승인) | 마켓 정책 게이트 |
...
- NAVER 리뷰 감독형 캡처 → 다운로드 저장 — 1회(2026-06-22; 자동 업로드 브리지 미검증).
