...
| **2026-08-20** | Coupang + NAVER | **Resident helper advertises and can serve FIVE carriers** — `issuance/coupang`, `issuance/naver`, `renewal/coupang`, `locate/coupang`, `import/naver` | `feat/full-product-integration-v1` | (product path — the seller's own press; nothing opens at activation) | `PARTIAL` — boot line verified live (`{"mode":"BRIDGE_ONLY","ok":true,…,"browserLaunched":false,"onDemandCarriers":[…5…]}`) and **0 helper-owned browser processes at idle**; activation of each of the five verified offline against a real socket (`local-agent-bridge-only.test.ts`). **NOT proven: a browser attach through the product screen** — pairing refused `approval_unavailable` because the helper was started without a TTY for its approval presenter, which is the pairing guard working. Needs one operator-attended pairing | `docs/channel_integration_completeness_audit_v1.md` §1, §3 |
