...
**WP-1(결정론 authority layer + eval 자산 복구) 구현됨 → `docs/inquiry_architecture_v3_wp1.md`. WP-2(Resolution Planner: contract → 201호출 shadow) → `docs/inquiry_architecture_v3_wp2.md`. WP-3(step shape를 capability class별로 쪼개고, effect를 registry로 옮기고, positional scorer를 split-tolerant goal scorer로 대체) → `docs/inquiry_architecture_v3_wp3.md` — plan gold는 v3.2로 재표현됐고(재라벨 0), `effect`와 `depends_on`은 plan에서 사라졌다. WP-4(Planner Final Validation & Freeze Gate) → `docs/inquiry_architecture_v3_wp4.md` — smoke 통과 후 67×1 shadow 실행, **freeze gate 미통과 → Resolution Planner는 동결되지 않았고 WP-4 E2E도 제안하지 않는다**(closing contract가 닫는 권한의 「개수」와 「순서」만 정하고 「무엇이 닫는가」를 정하지 않아, 마지막에 적힌 step이 closer가 된다 — 모델의 SELLER-마지막 습관 49/49가 의미 판단으로 바뀌었다). WP-3.1(Closing Authority Semantics & Replay Diagnostics) → `docs/inquiry_architecture_v3_wp31.md` — **모델 호출 0**, 기존 raw answer를 replay만 해서 진단했다(verdict 동일 확인). **WP-4의 headline은 방향이 틀렸다** — 「누가 계획에 있는가」가 아니라 「누가 실제로 닫는가」로 재면 v3는 1.000→0.875로 퇴행한 것이 아니라 **0.722→0.875로 개선**됐고, 1.000은 한 need가 두 개의 ending을 적은 plan 20/72건을 정답으로 세고 있었다. wrong closer 5건은 전부 **같은 모양**이다(gold가 닫아야 한다고 말한 authority가 PRECONDITION으로 강등되고 마지막에 적힌 step이 닫는다 — 그중 둘에는 SELLER가 아예 없다). truncated content 보존(`said`)·`unavailable_fields`·registry snapshot을 row에 적어 다음 truncation은 관찰 가능하지만 **기존 4건은 영구 복구 불가**다. closing contract 후보는 **C(`closing_authority`를 need의 명시 필드로)** 선택. **Part II에서 Candidate C를 구현했다**(모델 호출 0) — step-level role은 **삭제**됐고(`ResolutionPlan.Role` 부재, role을 실은 답은 `PLAN_SHAPE`), need가 `closing_authority`를 **steps보다 먼저** 선언하며, 같은 authority의 capability 둘이 함께 닫는 것이 한 필드로 표현된다(gold 4건·WP-4 실패 3건이 그 모양이었다). validator는 셋을 잃고(`NO_CLOSING_STEP`·`MULTIPLE_CLOSING_AUTHORITIES`·`PRECONDITION_AFTER_CLOSER` — 쓸 수 없게 됐으므로) 하나를 얻었다(`CLOSING_AUTHORITY_UNSUPPORTED`). gold **v3.3**은 v3.2의 무손실 재표현이고(양방향 검증 72/72, 재라벨 0) v3.2는 그대로 얼어 있다. **옛 모델 답변은 하나도 바뀌지 않는다** — projection으로 재해석하면 correct closer는 63/72로 동일하고, 바뀌는 것은 WP-2 needs의 **61/361(두 ending)**과 WP-4의 **3/5 contract violation**이 구조적으로 쓸 수 없게 된다는 것뿐이다. 남은 wrong closer 5건은 여전히 모델의 선택이며 **8-call smoke 전에는 아무것도 주장하지 않는다**. prompt `resolution-planner/v4` · backend 4,504/0 · tools 98/0 · mutation 19/19. **8-call smoke는 bar 8/8 통과**(commit `3397cc6d`, 호출 8/상한 8, 합성 질문만, 마켓플레이스·DB·마이그레이션 0) — correct closing authority 8/8 · contract violation 0 · seller fallback 0 · 신원 입력 0이고, 특히 `R:8989a9d0` 모양(C7)이 SELLER 없이 KNOWLEDGE로 닫혔으며 쓸 수 없는 capability(C8)는 **그대로 유지되고 gap으로 기록**됐다. bar가 이름 붙이지 않은 것 둘을 함께 보고한다 — C4에서 `ORDER_TRACKING` 한 칸 때문에 해결 가능한 goal이 gap이 됐고(§2의 `gapOf` 결정이 resolver 몫으로 남아 있는 그 결함의 두 번째 실물), C6은 고객이 묻지 않은 PROCEDURE need를 만들어 `procedure_for_read=1`이 됐다(freeze gate 3번 항목). **WP-3.2(Planner Boundary & Scope Hardening) → `docs/inquiry_architecture_v3_wp32.md`** — 모델 호출 0. residual 셋의 **소유자를 확정**했다. (A) PROCEDURE 경계: 72 gold를 먼저 감사해 「PROCEDURE step이 있는 goal 7건 전부가 PROCEDURE로 닫힌다, 반례 0」을 확인한 뒤에만 invariant `PROCEDURE_NOT_CLOSING`을 추가했다(WP-2에서 8회 발화했을 모양). **다만 이것은 C6를 고치지 못한다** — C6는 follow-up을 두 번째 need로 쪼갠 것이고 need 단위 규칙은 그것을 볼 수 없다. plan 단위 규칙은 정당한 계획(예외 승인과 반송 방법을 함께 묻는 문의)을 금지하게 되므로 만들지 않았고, **계약의 한계로 기록**한 뒤 instruction 한 문장과 `procedure_for_read` 측정에 맡긴다. (B) scope: capability만 보던 채점기가 C3/C7의 `SELLER_CATALOGUE`→`THIS_LISTING`를 0으로 보고하고 있었다 ⇒ scope/instance를 **first-class metric**으로 올리고(denominator는 선택지가 둘 이상인 capability만) freeze gate에 넣었다 — 실측 capability 11/11, **scope 0/2**. (C) over-read: `gapOf`는 **건드리지 않고** 설계 셋을 값으로 비교해 **B(planner field는 후보, resolver가 최소 충분 state를 결정)**를 WP-4 Entity Resolver 입력으로 권고한다(A에서만 P01·C4 두 건이 해결 가능한 goal을 gap으로 만든다; field-level은 R:7a8136b2 반례로 여전히 기각). prompt는 `resolution-planner/v5`로 바뀌었고 **system_fp 변경·schema_fp 불변·input_fp 8/8 동일·request_fp 0/8 동일** ⇒ **벤더 요청이 바뀌었으므로 67×1 전에 새 8-call smoke가 필요하다**(manifest 제시, 승인 없음). backend 4,504/0 · tools 106/0 · mutation 22/22. **8-call v5 smoke 실행 결과 bar 7/8 — 이 패키지가 존재한 이유인 3번 항목에서 실패**(`C6`가 v5에서도 PROCEDURE follow-up need를 만들었다; closing authority는 8/8, scope는 2/3). 결과를 판정하기 **전에** 채점기 결함 하나를 고쳤다 — C7이 두 need에 THIS_LISTING과 SELLER_CATALOGUE를 모두 적었는데 비교가 먼저 나온 step을 집어 「잘못된 scope」로 보고하고 있었다(scope 1/3→2/3, 남은 C3는 진짜 오답). v4 대비 7개 공유 case는 **user turn이 전부 동일**하고, 경계 문장은 reach를 줄이지 않고 **재배치**했다(2건 축소·3건 확대·2건 불변, step 11→13) — C6에서는 불필요한 주문 4필드 읽기를 없앴지만 PROCEDURE need는 그대로다. arm당 표본 1이므로 일반적 우열은 주장하지 않는다. 분류는 **boundary**이고 **67 case는 돌리지 않으며 prompt 재튜닝·재시도도 하지 않는다**. 구조적 선택지는 소진됐다(per-need 규칙은 추가했고 split을 볼 수 없으며, plan 단위 규칙은 정당한 계획을 금지한다, instruction은 한 번 시도해 측정됐다) ⇒ 남은 것은 **WP-4 E2E가 답할 제품 질문**이다(PROCEDURE는 어디서나 `DECLARED_NO_EXECUTOR`라 잘못된 **행동**은 불가능하지만 잘못된 **문장**은 가능하다). 67×1을 이 잔여를 진단으로 안고 진행할지는 **product-owner 결정**. planner는 여전히 동결되지 않았고 WP-4 E2E도 제안하지 않는다.** **Inquiry v3.5(Customer Goal Interpreter + Deterministic Resolution Loop) → `docs/inquiry_architecture_v35.md`** — 모델 호출 0 · 마켓플레이스 0 · DB 0 · 마이그레이션 0. **Full Resolution Planner를 `EXPERIMENTAL / NOT_ADOPTED_FOR_PRODUCTION`으로 기록**하되 삭제하지 않는다(코드·프롬프트 v1~v5·fixture·기록된 run 전부 comparison/replay asset으로 유지; 네 타입에 상태 배너를 넣고 `LegacyPlannerStatusTest`가 배너 부재와 production caller 등장을 빌드 실패로 만든다). **채택하지 않은 이유는 「authority 분리 실패」가 아니라 「책임 과다」**다 — 한 번의 모델 호출이 *고객이 지금 무엇을 요구했는가*와 *그것을 만족시킬 미래 workflow 전체*를 동시에 정했고, 서로 달라 보이던 실패 다섯(과잉 field read · 과잉 customer input · 요청하지 않은 future procedure · 보험용 SELLER · 성급한 scope)은 전부 **「아직 일어나지 않은 일 다음에 무엇이 오는가」를 모델에게 물은 것**이었다. 감사 결과 **planner는 production caller가 0**이므로 (유일한 seam은 package-private이고 `src/test`에서만 도달) 「DEFAULT OFF」는 이미 가장 강한 의미로 참이다. 대체물은 `CustomerGoal`(id · explicit_request · requested_outcome · subject · basis · explicit_constraints)이고 **금지 출력은 규칙이 아니라 shape로 막는다** — fields · customer_inputs · capability sequence · procedure · fallback · handoff · closing_authority · availability · effect를 담을 칸이 아예 없고, wire mirror는 그 이름들을 `GOAL_PLAN`으로 거절한다. `requested_outcome`은 **INFORMATION / STATE_READ / DECISION / ACTION 넷**이고 이것은 가정이 아니라 측정이다 — **72 frozen goal 전부가 mapping되고(unmappable 0), 60건이 판단 없이 확정되며, 다섯 번째 값이 필요한 행은 0건**이다(`contracts/inquiry-customer-goal/v1`, 모델 호출 0). 그리고 그 mapping은 **반증 가능한 검사**를 통과한다 — 새 vocabulary가 frozen `closing_authority`를 dispatch 표 그대로 예측하는가를 build가 단언하고 **확정 60건 60/60 일치**다. **가장 중요한 발견: frozen gold의 PROCEDURE-closing goal 7건 중 5건이 adjudication row**다 — planner가 계속 넘던 DECISION/ACTION 경계는 **이 코퍼스도 확정한 적이 없는 경계**였고, 그래서 어떤 prompt tuning으로도 닫히지 않았다(미확정 12건은 6개 class로 제시하며 추측하지 않는다 — **product-owner 결정**). runtime은 plan이 아니라 loop다: `ResolutionPolicy.next(goal, observed)`가 **한 걸음만** 돌려주고(`Dispatch`는 `Run | Settle`로 sealed이라 sequence를 담을 shape가 없다), **ACTION만 procedure에 닿을 수 있으며**(모든 outcome × 모든 referent 전수 단언 + mutation), **DECISION은 KNOWLEDGE를 먼저 묻고 관측된 부재에서만 SELLER에 닿는다**(SELLER는 fallback router가 아니라 요구된 judgment). **capability 부재는 의미를 바꾸지 않는다** — 「돌았는데 아무것도 없다」는 `NEEDS_SELLER`, 「돌 수 없었다」는 registry 이유를 그대로 실은 `CAPABILITY_GAP`이다. **새 terminal state는 0개** — 후보 7 중 6은 이미 `ResolutionState`에 있었고 나머지 하나는 `GapReason.ACQUIRABLE`이며, `ACTION_REQUIRES_APPROVAL`은 **생산자가 없어 만들지 않았다**(모든 procedure가 `DECLARED_NO_EXECUTOR`). over-read는 `EntityStateRequirement`(required **dimension** — 어느 한 field로 충족)로 **두 regression을 동시에 통과**한다(P01·C4 무해한 over-read가 더 이상 치명적이지 않고, `R:7a8136b2`의 진짜 결손은 여전히 gap이며 「읽히는 아무 field나」는 거절) — 다만 **`gapOf`는 이번에도 바이트 동일**이고 resolver 재배선은 Phase D다. 외부 조사(공식 문서, 신뢰도 표기): **어떤 공식 자료도 LLM이 해결 전체를 미리 계획한다고 기술하지 않는다** — 세 제품 모두 **LLM은 entry, 사람은 body**를 소유하고 선언된 procedure가 곧 plan이며, 두 엔지니어링 가이드는 loop를 observe-then-decide로 기술한다(다만 「그렇게 하지 않는다」고 말한 벤더도 없다 — 도움말 문서는 authoring surface이지 내부 아키텍처가 아니다). backend **4,533/0**(54 skipped) · tools **109/0** · mutation **27/27**. Goal Interpreter LLM 구현은 **일부러 하지 않았다** — 12건 adjudication이 확정되기 전에 prompt를 쓰면 미확정 라벨이 굳는다. 다음 단계는 **12-call Goal Interpreter smoke manifest**(§20, 승인 없음·미실행)이고 그 앞에 adjudication A·B·C 확정이 선행된다. 67×1도 WP-4 E2E도 제안하지 않는다. **A/B/C adjudication 완료(2026-09-20, product-owner) → gold v2**: 9건이 확정돼 emitted goal 68 · NO_GOAL 1 · 미결 3(D/E/F). NO_GOAL은 **무시가 아니다** — 케이스는 남고 VOC/불만 신호는 다른 곳에서 나며, 다만 이 loop가 풀 goal이 없다는 뜻이다(scorer는 NO_GOAL row를 배정 불가로 만들어 거기에 나온 goal을 **정의상 invented**로 센다). 확정 결과 **frozen resolution gold 5행이 더 이상 따라오지 않는다**(GOAL_OBSOLETE 1 · CLOSER_MOVES 2 · PLAN_OBSOLETE 1 · NO_CAPABILITY 1) — build는 이제 「일치한다」를 단언하지 않고 **「모든 불일치가 선언돼 있다」**를 단언하며, 그 5행을 여기서 **재도출하지 않는다**(별도 결정). 특히 `S:N2`는 이 registry가 **gap 이유로도 표현할 수 없는** capability를 가리킨다(세금계산서 procedure 자체가 없어 `DECLARED_NO_EXECUTOR`로도 적을 수 없다). 그리고 지시받은 **DECISION prerequisite audit이 가정을 깼다** — frozen gold의 기록된 step으로 재 보니 **DECISION 5건 중 3건이 판단 전에 관측된 ENTITY_STATE를 요구**하고, 그중 둘(`S:T7a`·`S:T7b`)은 **`S:T12a` 재분류 이전부터 gold에 `ENTITY.LISTING`이 적혀 있었다**. 그 감사가 **잠복 결함을 찾아냈다** — resolver가 prerequisite를 지목하고 그 prerequisite가 **성공하면**, loop가 prerequisite의 결과에 정착하고 **물어본 resolver로 돌아가지 않았다**(실측: 주문 상태가 필요한 DECISION이 판단을 한 번도 하지 않고 `RESOLVED`를 보고했다; `G12`가 못 잡은 이유는 그 fixture의 prerequisite가 **실패**하기 때문이다). **dispatch 표는 바꾸지 않았다** — DECISION이 KNOWLEDGE를 먼저 묻는 것은 옳다(결정 규칙이 knowledge이고 규칙이 자기 prerequisite를 선언한다) — 바뀐 것은 **resume 규칙 하나**이고 이는 planner가 아니다(prerequisite는 여전히 resolver가 **돈 뒤에** 지목하고, gap난 prerequisite는 여전히 goal을 끝낸다). fixture `G13`·`G14`는 **수정 전 코드에서 빨개지는 것을 확인**했다. backend **4,537/0** · tools **111/0** · mutation **28/28**. D·E는 계약 확장 없이 판정 가능하고 **F만 계약 결정**이다(고객이 말한 대안 순서를 담을 칸이 없다). smoke는 **14 call**로 갱신됐고 여전히 **승인 없음·미실행**이다. **D/E/F adjudication 완료 → gold v3(2026-09-20)**: 미결 0. **D**(`R:f81ad84a` 「모든 박스의 송장번호 전달 부탁드립니다」)는 **STATE_READ** — 아직 없는 배송 상태를 묻는 모양은 `R:7a8136b2`·`S:T13a`가 이미 STATE_READ로 확정했고, 남는 차이(「나중에 알려 달라」)는 CustomerGoal이 **어느 행에 대해서도** 표현하지 않는 답변 전달 시점이다; adjudicated ACTION들과 가르는 기준은 **주문을 바꾸는가 vs 관측된 사실을 전달하는가**이고 frozen gold도 이 행을 ENTITY_STATE로 닫는다. **E**(`R:f403e606 n2` 「유리가 더 좋은 거 아닌가요?」)는 **INFORMATION / SELLER_CATALOGUE** — 제품 비교·추천은 `S:T10a`·`R:77a91fab n3`의 선례대로 사실이고, DECISION은 전부 **판매자 자신의 행위**를 묻는 행이다(referent는 MEDIUM confidence로 기록: 고객이 이름 댄 상품이 경쟁사 것이면 UNRESOLVED가 옳다). **F**(`R:515dd536` 「노즐만 보내주시고, 노즐만 배송이 불가능하면 환불처리」)는 **label이 아니라 계약 문제**였고 최소 계약 하나로 닫았다 — `CustomerGoalSet{goals, relations}` + `GoalRelation{FALLBACK, primary, fallback, statedCondition}`. **planner 재도입이 아닌 이유는 구조다**: kind가 FALLBACK 하나뿐이라 실행 순서를 표현할 수 없고, **`statedCondition`(고객 원문 인용)이 필수라 조건을 말하지 않은 두 ACTION은 관계를 만들 수 없으며**, capability·step·gap을 담을 칸이 없고, `ResolutionPolicy`에는 relation을 받을 파라미터가 없다(구조 테스트). **그리고 fallback은 절대 자동 활성화되지 않는다** — `CAPABILITY_GAP`은 「업무적으로 불가능」이 아니라 「이 배포가 시도조차 못 했다」이고, 이 registry의 모든 procedure가 `DECLARED_NO_EXECUTOR`라 **모든 ACTION goal이 gap으로 끝나므로**, gap을 거절로 읽으면 조건문을 쓴 모든 고객이 환불을 받는다; 권위 있는 거절을 낼 **생산자가 이 시스템에 없으므로** runtime은 `WITHHELD_FOR_CUSTOMER_STATED_CONDITION`으로 **resolver를 아예 부르지 않고** 고객의 조건과 함께 판매자에게 넘긴다(모든 terminal × 모든 GapReason 전수 단언). **waiter/resume을 state machine A~I로 굳히다가 같은 규칙의 두 번째 결함을 찾았다** — 09f34a28의 수정은 `CAPABILITY_GAP`에만 걸려 있어서, prerequisite가 **돌았는데 아무것도 없을 때**(NEEDS_SELLER)·고객 입력을 요구할 때·실패할 때는 전부 resume으로 떨어져 **묻지 않은 답을 받은 것처럼** waiter가 재실행됐다(`G19`·`aBlockedPrerequisiteIsNeverResumedPast`를 `09f34a28`에서 red 확인). nesting fixture가 두 번째 버그도 드러냈다 — resume이 prerequisite의 **첫** 발화를 읽고 있었다(두 번 말한 prerequisite에서 잘못된 결과를 본다) ⇒ **마지막** 발화로. depth 상한은 `CapabilityId.values().length - 1`로 **registry에서 파생**한다(고른 숫자가 아니다). **DECISION resolver 계약**도 다시 적었다 — 「knowledge, 없으면 seller」가 아니라 **판단 규칙을 찾고, 그 규칙이 필요하면 prerequisite 하나를 좁게 요구하고, 관측 뒤 규칙으로 돌아온다**. regression 셋 고정: **S:T12a**=DECISION(entity read는 resolver prerequisite이고 procedure 생성 0) · **S:T10b**=INFORMATION(resend/refund/보상/order action 발명 0) · **S:N2**=ACTION이 `CAPABILITY_GAP/NOT_SUPPORTED`로 끝나고 **KNOWLEDGE로 의미 치환되지 않는다**(새 gap reason은 검토 결과 **불필요** — `NOT_SUPPORTED`가 이미 그 주장이다). gold v3: 72 goals · NO_GOAL 1 · multi-goal 1 · explicit FALLBACK 1 · **미결 0** · legacy conflict 7(신규 `SCOPE_MOVES`·`GOAL_SPLITS`, 전부 선언·**재도출 0**); build 검사는 약해지지 않고 **강해졌다**(closer에 더해 **instance** 불일치도 검사하고, 실재하지 않는 선언도 실패시킨다). eval에 **relation fidelity · invented relation rate**와 safety blocker 다섯을 추가했다 — goal·outcome·referent가 **전부 맞아도** 고객의 순서를 버리면 기존 지표는 1.0이고 `lost_stated_fallback`만 1이다. **Goal Interpreter prompt/schema를 작성**했다(`customer-goal-interpreter/v1`, payload floor가 planner의 4사실 → **2사실**로 더 좁아졌고 availability는 여전히 미포함) — fingerprint는 `contracts/inquiry-goal/v1/prompt-fingerprint.txt`에 고정돼 테스트가 대조한다. backend **4,564/0** · tools **124/0** · mutation **33/33** · fixture 14 → **23**. **기존 12/14-call smoke manifest는 STALE로 표시**했고(계약이 바뀌었고 그때는 prompt가 존재하지도 않았다) §22.11의 **14-call · bar 12/12**로 대체했다 — 여전히 **승인 없음·미실행**이며 모델 호출 0 · 마켓플레이스 0 · DB 0 · 마이그레이션 0. 아래 본문은 설계 당시 그대로다 — WP-1이 바꾼 것 둘: GapReason에 가능한 gap `UNREADABLE_SOURCE`가 더해졌고(§4-B의 ACQUIRABLE과 같은 「가능한 gap」), 부록 A의 계획 라벨은 plan gold v3로 동결됐다(6행 PENDING_ADJUDICATION).
