...
- **남은 FE progress/resume 공백(범위 밖, backend-only):** 백엔드가 이제 `manualSync`에서 coalesce 시 RUNNING `SyncRunView`를 반환할 수 있으나, 가이드 위저드 `runFirstSync`는 결과를 terminal sync 결과로 취급 — "이미 진행 중" 상태가 FE에 없고, ~8.6분 동기 sync 스피너(재-트리거 유발 원인)도 그대로. 이는 별도 **frontend 단위**.
