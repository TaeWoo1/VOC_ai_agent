...
## 17. Grouped Product Answers v1 — B1 수정 (2026-08-24)

> §3·§4의 baseline 숫자는 이 절로도 바뀌지 않는다. A1·A2·A4·A5·A8·A9·C1·C3·C4·C5는 그대로 닫혀 있고,
> 이 절은 그 위에서 **셀러의 질문 축**을 하나 추가한다.

### 17.1 무엇이 틀렸나 — B1

A9/C5가 닫힌 뒤 run은 범주 명사 때문에 잘못 좁히지 않았다. 그런데 「최근 부정적인 리뷰가 있는 **상품**을
알려줘」에 대한 답은 여전히 이랬다:

> "접착 탈락"에 대한 리뷰 근거가 19건 기록돼 있습니다.

문장의 모든 숫자가 참이고, 모든 근거가 실재하며, **어느 것도 상품에 관한 것이 아니다.** 셀러는 자기 상품
중 무엇을 봐야 하는지 물었고 답은 이슈 목록이었다. 이슈 축과 상품 축은 다른 축이고, 이슈 목록을 더
정성껏 읽어도 상품 축은 나오지 않는다 — 이슈 행의 모든 수치는 **org 전체**의 것이기 때문이다(C4).

### 17.2 먼저 감사 — 기존 READ가 실제로 무엇을 아는가

새 retrieval을 만들기 전에, 이미 있는 읽기가 상품 축을 감당하는지 확인했다. 전부 **라이브 데모 org에서
직접 확인**한 사실이다(2026-08-24, 마켓 접촉 0).

| read | 실제 의미 | product id | channel id | 세분성 | 완전성 | 판정 |
|---|---|---|---|---|---|---|
| `get_review_issue_evidence_summary` | 한 이슈의 근거를 **상품별로 나눈 집계** + 미귀속 수 + 별점 분포 + 이슈 전체 기간 | **있음** (canonical) | 없음 | 이슈×상품 count | 이슈 단위 완전 | **상품 축의 유일한 근거** |
| `search_unanswered_inquiries` | phase=OPEN 대기열 한 페이지 — workItem/inquiry id, 채널 **UUID**, 상태, 제목, 접수 시각 | **없음** | 코드 아님(UUID) | 행 목록 | `totalElements`로 자기 완전성 진술 | 우선순위 근거로 연결, **상품 축 불가** |
| `list_item_analysis` | 저장된 분석 4,411행 — sourceType/sourceId(리뷰·문의 id), 카테고리, 감성, 긴급도 | **없음** | 없음 | 행 목록, 페이지 없음 | 4,411행 무제한 | 연결 안 함 |
| `get_dashboard_product_issues` | **상품별 부정 리뷰 수 상위 5** (`Review.isNegative` group by productId) | **없음** — DTO가 `productName`만 | 없음 | 상품×count | 상위 5, 기간 필터 없음 | 연결 안 함 — 아래 |

**대시보드 rollup은 정확히 「상품별 부정 리뷰 수」이고, 그래서 더 아깝다.** 라이브에서 상위 5개 합이
12건인데 org 부정 리뷰는 22건이고, 무엇보다 행에 **canonical product id가 없다**. 데모 org에는 같은 이름을
가진 상품이 10쌍, 한 이름이 4개 상품에 붙은 경우까지 있다(§13.3) — 이름을 키로 묶는 순간 서로 다른 상품이
한 행이 된다. §6이 요구하는 「canonical product identity」를 만들 수 없으므로 근거 행으로 쓰지 않았다.
**backend DTO에 id 한 줄을 더하면 그날 쓸 수 있다** — 이번 범위에서 backend를 넓히지 않았을 뿐이다.

**그래서 「정확한 상품별 부정 리뷰 랭킹」은 이번에도 없다.** 있는 것은 **리뷰 문제 근거의 상품별 집계**이고,
그것을 「부정 리뷰 수」로 개명하지 않는다 — 다른 것을 세는 숫자다.

### 17.3 축은 무엇이 정하는가

`GroupingDimension = NONE | PRODUCT`. 한 run에 한 번 정해지고(`group/ProductGrouping.ts`), specialist는
그것을 **받는다** — 각자 해석하면 「상품별」의 뜻이 두 곳에서 생긴다.

두 입구, **어휘는 하나**(`plan/EntityRole.ts`가 저장소의 유일한 범주 표):

1. planner가 선언한 **PRODUCT CATEGORY mention** — 「상품별」, 「전체 상품」.
2. **셀러 자신의 문장** — 핵어가 축 조사(별/이/가/을/를/은/는/도/들)를 달고 있을 때.

두 번째가 필요한 이유는 측정된 것이다: 2026-08-24 Q2를 3회 샘플링했을 때 planner는 `PERIOD`와 `ISSUE`만
선언했고 **PRODUCT entity는 3회 모두 없었다.** 계획만 읽는 축 판정은 Q2·Q3에서 한 번도 발동하지 않는다.
조사를 요구하는 것은 안전 방향이다 — 「상품**의**」(속격)과 「상품**에**」(처격)는 축이 아니고, 그래서
「상품에 문제 있어?」는 지금도 어느 상품인지 되묻는다.

**INSTANCE는 항상 축을 취소한다.** 이름을 말했거나 해결된 상품이 있으면 grouping은 `NONE`이다. 「판도리
일체형 종이컵 수거함 **상품의** 리뷰」는 랭킹 요청이 아니고, 그것을 랭킹으로 답하는 것은 A1을 옷만 갈아입혀
되살리는 일이다.

**grouped row는 entity가 아니다.** 행은 canonical `productId`를 갖지만 `state.entities`에 들어가지
않는다 — 해결된 entity는 run 전체의 scope를 좁히므로(`needScopeOf`), 11개 상품을 묶은 run이 그 11개를
entity로 기록하면 같은 답 안의 org 근거가 전부 불법이 된다.

### 17.4 grouped row 계약

각 행: canonical id · 사람이 읽는 label(C1) · 그 상품 **자신의** count · 기여한 이슈 목록 · 최대 기여 이슈 ·
증명된 기간 · 채널(근거가 실제로 실어 온 경우에만 — 오늘은 항상 없음).

- **키는 언제나 canonical id.** 같은 이름 두 상품은 두 행이고, 한 상품은 몇 번 나와도 한 행이다.
- **(issue, product) 쌍은 한 번만 센다.** 재조회가 수치를 부풀릴 수 없다.
- **순위는 그 상품의 수.** 이슈 목록의 순서(심각도 → 이슈의 org 수)는 상품의 중요도가 아니다 — C4를 한
  층 위에서 다시 적용한 것이다.
- **이름 없는 상품은 지우지도, 이름 짓지도 않는다.** 라이브 11개 중 3개가 카탈로그에 이름이 없다. 그
  3개 11건은 「상품명을 확인할 수 없는 3개 상품」으로 **수치와 함께 고지**된다.
- **어느 상품에도 붙지 않은 근거**(`unattributedEvidence`)는 따로 고지한다. 나눠 갖지 않는다.

### 17.5 스캔은 자기 크기를 말한다

이슈 19건을 다 여는 것은 예산 밖이므로 **근거가 많은 8건**을 연다. 순서를 정한 근거도 측정이다: 근거 수
상위 8건이 85건 중 **71건(84%)**, 심각도 상위 8건은 44건(52%)이다.

그리고 그 사실이 **답에 들어간다** — 로그가 아니라:
...
상품별 분할에는 **날짜가 없다**. 이슈의 기간을 상품 행에 빌려주면 남의 상품의 최근 리뷰가 이 상품의
「최근」을 증명하게 된다(C4의 시간 축 버전).

증명되는 경우가 하나 있다: **이슈의 근거가 전부 한 상품의 것이고 미귀속이 0이면, 그 이슈의 기간이 그
상품의 기간이다.** 그때만 행에 기간이 실리고, 행의 count를 이룬 **모든** 조각이 그런 경우일 때만 실린다 —
일부만 날짜가 있으면 보이지 않는 나머지에 날짜를 붙이는 셈이기 때문이다.

**그래서 기간을 묻는 질문에서는 상품 행이 대부분 보류된다.** 라이브 Q2에서 9개 상품 전부가 그랬다(상위
4개 이슈 58건이 다상품 이슈다). 답은 그것을 **말한다**:

> 이 가운데 9개 상품은 근거가 언제 발생했는지 확인할 수 없어, 기간을 묻는 이 질문의 답으로는 상품별
> 수치를 제시하지 않았습니다.

이것이 지금 남은 가장 큰 blocker이고, §17.10이 그 이름을 댄다.

### 17.7 없는 축은 근사하지 않고 말한다

`GROUPING_CAPABILITIES`는 (need kind × dimension)의 실행 가능 여부를 **구조로** 선언한다. 「못 한다」가
두 종류라서 어휘도 둘이다 — 어느 쪽인지가 다음 package가 무엇을 고쳐야 하는지를 말해 주기 때문이다.

| need | dimension | 판정 | 근거 |
|---|---|---|---|
| `REVIEW_SIGNAL` | PRODUCT | **SUPPORTED** | `evidence-summary:byProduct[productId,evidenceCount]` |
| `INQUIRY_VOLUME` | PRODUCT | `NO_GROUPED_READ` | `Inquiry.productId`는 **있고** 상품 단위 집계도 있지만, 상품을 먼저 지명해야 한다. 대기열 행에는 상품이 없다 |
| `REPEAT_PATTERN` | PRODUCT | `NO_PRODUCT_ATTRIBUTION` | `RepeatedInquiryView`에 상품 필드가 **아예 없다** — 반복은 signature 군집이고 군집이 상품을 들고 온 적이 없다 |

선언되지 않은 쌍은 `NO_GROUPED_READ`로 닫힌다. 런타임은 need도 specialist도 스스로 만들지 않는다.

### 17.8 우선순위는 총계가 아니다 — 대기열 연결

Q5는 「우선순위대로 정리」를 요구하고 답은 「69건 있습니다」였다. `search_unanswered_inquiries`를 연결해
**행의 자기 날짜**로 순서를 만든다. 임의 순위는 만들지 않는다 — 접수 시각은 행이 증명하는 근거다.

**고객 문장은 한 글자도 지나가지 않는다.** 대기열 행의 `title`은 읽지 않는다(회귀가 지킨다).

**두 수를 화해시키지 않고 라벨을 붙인다.** `get_today_inbox`는 status=UNANSWERED를 세어 **69**,
대기열은 phase=OPEN을 세어 **68**이다(라이브 확인). 어느 쪽도 「그 숫자」가 아니므로 각각이 무엇을 셌는지
말하고, 다르면 다르다고 고지한다.

> **범위 밖에서 발견한 것, 보고만 함.** 홈 대시보드는 같은 질문에 **20**을 표시한다
> (`countByOrgIdAndStatusExcludingSecret` — 비밀글 제외). Operator는 69(비밀글 포함)를 말한다. 둘 다
> 의도된 계산이지만 **셀러 한 명이 두 화면에서 다른 수를 본다.** 어느 corpus가 「답변이 필요한 문의」인지는
> product-owner 결정이므로 이번 package에서 바꾸지 않았다.

### 17.9 회귀와 red 증명

`groupedProductAnswers.test.ts` **28건 신규**. agent-runtime 전체 **454 passed / 23 skipped / 0 failed**
(operator suite 305), `tsc` clean. 백엔드·프론트엔드 변경 0, UI 0, 새 retrieval tool 0.

각 보호장치를 하나씩 되돌렸을 때 실제로 붉어진 개수:

| 되돌린 것 | red |
|---|---|
| **INSTANCE가 축을 취소하지 않음**(A1 위험 방향) | 2 |
| 축을 계획에서만 읽음 — 셀러 문장 무시 | **3** |
| 조각 하나만 날짜가 있어도 행에 기간을 부여 | 2 |
| 순위를 상품 자기 수가 아닌 것으로 | 1 |
| canonical id 대신 **이름으로 묶음** | **3** |
| grouped path 자체를 끔 | **3** |
| 스캔 고지 문장 제거 | 1 |
| 상품 수 대신 이슈 총계를 진술(C4 방향) | 2 |
| 반복 축 한계 고지 제거 | 1 |
| 대기열 읽기 제거 | **4** |
| 대기열 제목을 문장에 넣음(고객 텍스트 유출) | 1 |
| 선언되지 않은 (need × 축)을 SUPPORTED로 | 1 |

A1·A2·A4·A5·A8·A9·C1·C3·C4·C5·temporal·READ-only fence는 모든 red run에서 초록을 유지했다.

**의미가 바뀐 기존 회귀 1건, 명시.** `evidenceScopeIntegrity` Q1의 「목록 need는 카운트로 만족되지
않는다」는 v3까지 *n2가 만족되지 않음*으로 확인했다. 대기열 읽기가 생겼으므로 이제 n2는 **만족된다 —
행으로**. 축(카운트 ≠ 목록)은 그대로이며, 같은 파일에 단위 회귀로 다시 못 박았다: `INBOX_COUNT`를 목록
need에 주면 `GRANULARITY_MISMATCH`, `INQUIRY` 행이면 통과.

### 17.10 라이브 — REAL Demo Org (2026-08-24, 마켓 접촉 0, WRITE 0)

**축이 실제로 상품을 짚는다.** 「어느 상품에 리뷰 문제가 가장 많아?」 2회, **두 run이 같은 답**:

> 선바로 일체형 전선몰딩 열고 닫기 편한 전선몰드에 리뷰 문제 근거가 **48건** 기록돼 있습니다
> (가장 많은 것은 "접착 부족" 16건, 확인한 문제 8건 합계).
> 나누리샵 세모금컵 디스펜서 커플미니에 **7건** … 종이컵보관함 수거함 디스펜서 컵 홀더에 **2건** …
> 세모금 생수컵 디스펜서 보관함 5종에 **1건** … 세모금컵 4000매 …에 **1건**.
> 열려 있는 반복 리뷰 문제 19건 가운데 근거가 많은 8건(전체 근거 85건 중 71건)을 확인해 상품 9개로
> 나눴습니다. 나머지는 확인하지 않았으므로 전체 순위가 아닙니다. 상품명을 확인할 수 없는 3개 상품의
> 11건은 이름 없이 남겨 두었습니다.

`review_ops_grouped` 로그 3회 모두 `issues:19 checked:8 products:9 unnamed:3 unattributed:0 dated:0`.
`resolve_product` **0회**(C5 유지). tool 9회(목록 1 + 요약 8), `nextActions` 4건 전부 `READ`.

**독립 검증.** 같은 org를 API로 직접 집계한 결과 — 이슈 19건, 근거 85건, 상품 11개, 미귀속 0 — 이고
상위 상품은 선바로(전체 19건 스캔 시 57건)다. run이 말한 48건은 **8건만 열었을 때의 그 상품 몫**이며,
문장이 그 사실을 함께 말한다.

**기간을 물으면 상품 행은 보류된다(그리고 그렇다고 말한다).** Q2·C5 모두 9개 상품 전부 `dated:0` —
상위 4개 이슈(85건 중 58건)가 다상품 이슈여서 상품별 기간이 증명되지 않는다.

**축이 없는 need는 축이 없다고 말한다.**

| 프롬프트 | 답 |
|---|---|
| 「반복해서 비슷한 문의가 들어오는 상품이 있어?」 ×2 | 「반복 문의 기록에는 상품 정보가 없어 상품별로 나눌 수 없습니다. 상품별로는 지금 답할 수 없습니다.」 + org 기준 28일 0건 |
| 「상품별 미답변 문의 알려줘」 | 「미답변 문의 목록은 상품별로 모아 볼 수 있는 조회가 아직 없어 상품별로 나누지 못했습니다.」 + 69/68/30일 초과 68 |

이전 v3에서 Q3의 답은 「최근 28일 반복 0건」 하나였고, 셀러의 질문(「상품이 있어?」)에 대한 답으로 읽혔다.

**우선순위가 생겼다.** Q5·Q1·Q6 전부:

> 답변 대기열에서 68건을 확인했고, 접수 순서대로 보면 **2014-10-28에 접수된 건이 가장 오래 기다렸습니다.**
> 그중 68건은 접수된 지 30일이 넘었습니다.

### 17.11 v3 → v4 (Q1~Q6, 동일 문장, 각 2회 · 축 프롬프트 별도 6회)

| # | v3 | v4 | 판정 |
|---|---|---|---|
| Q1 | finding 4 | finding 6/3 — **대기 순서와 30일 초과 건수 추가** | 개선 |
| Q2 | 이슈 3건 | 이슈 3건 + **상품 축을 만들고, 기간을 증명 못 해 보류했다고 진술** | 부분 개선 |
| Q3 | 「최근 28일 반복 0건」 | **「상품별로 나눌 수 없습니다」 + org 0건** | 개선(정직) |
| Q4 | 상품 해결 · 측정된 0 | **동일 2/2** (축 발동 0 — INSTANCE가 취소) | 유지 |
| Q5 | 69건 | 69건 + **68건 대기열 · 가장 오래 기다린 건 · 30일 초과 68건** | 개선 |
| Q6 | finding 4~6 | finding 8/6 (대기열 문장 포함, REPORT_OPS 합성) | 개선 |

| rubric | v3 | v4 |
|---|---|---|
| product grouping 요청 인식 | 없음 | Q2·Q3 및 축 프롬프트 **6/6** |
| grouped products returned | 0 | 기간 없는 질문 **5개 상품(2/2 동일)**, 기간 질문 0(보류 고지) |
| product label | — | 카탈로그 이름 그대로(C1). 이름 없는 3개는 **수치와 함께 고지** |
| product counts | — | 그 상품 몫만. 이슈 총계 인용 0 |
| org vs product scope | 전부 org | run은 org, **답만** 상품별. entity 축 거절 0 |
| limitations | 기간 기본값 고지 | + 스캔 8/19·71/85, 미명명 3, 축 불가 2종, 기간 미증명 |
| evidence completeness | 이슈 목록 whole | 스캔 비율을 **문장에** 표기 |
| unsupported claims | 0 | **0** |
| actionable usefulness | 「이슈가 있다」 | 「이 상품부터」 + 「이 문의부터」 |
| WRITE | 0 | **0** (nextActions 전부 READ, 카탈로그에 WRITE 없음) |
| 되물음 | 0/12 | **0/12** |

### 17.12 남은 것 — 그리고 다음 blocker 하나

- **가장 큰 blocker: 상품별 근거에 날짜가 없다.** `IssueProductEvidenceView`는 `(productId,
  productName, evidenceCount)`뿐이고, 그 뒤의 `review_issue_evidence` 행에는 `occurred_on`이 **이미
  있다**. 그 한 필드(상품별 first/last)가 실리면 「최근 부정 리뷰가 있는 상품」이 오늘 보류하는 답을
  그대로 낸다. 지금 이 한 가지가 Q2를 막는 유일한 것이다.
- **정확한 「상품별 부정 리뷰 수」**는 `topProductIssues`에 있으나 canonical id가 없어 쓰지 못한다
  (§17.2). DTO에 id 한 줄.
- **cross-channel** 6/6 그대로. 근거 행에 채널이 없다 — grouped row의 `channelCode`는 오늘 항상 null이고,
  추측하지 않는다.
- **반복 문의의 상품 축**은 데이터 모델이 없다(D2 유지).
- **미답변 69 vs 홈 20** — §17.8, product-owner 결정.

### 17.13 판정

| 결함 | 상태 |
|---|---|
| **B1** — 상품 축 요청을 org 축으로 답함 | **CLOSED (REVIEW_SIGNAL)** — 축·행·스캔 고지가 구조로 존재하고 라이브로 상품을 짚는다 |
