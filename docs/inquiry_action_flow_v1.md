...
| CAFE24 | (단일) | **`NEEDS_VERIFICATION`** | 플랫폼은 확인됨 — 공식 사본 `docs/vendor/cafe24-admin-api/post-boards-articles-comments.md`: `POST /api/v2/admin/boards/{board_no}/articles/{article_no}/comments`, scope `mall.write_community`, **필수 `content`·`writer`·`password`** · 미확정 ①board 6 댓글이 `reply_status`를 바꾸는지 근거 없음 ②`writer`/`password`를 SellerOps가 보유하지 않음 ③**2026-08-25 재취득으로 정정됨 — 후보는 하나가 아니라 셋이다**(사본 `docs/vendor/cafe24-admin-api/get-boards-articles.md` 재취득·정정본 · `docs/vendor/cafe24-admin-api/urgentinquiry-and-reply.md` 신규 · 설계 `docs/inquiry_answer_execution_v1.md`): `PUT /articles/{article_no}`가 `reply_status`를 받지 않는다는 관측은 영문·국문 두 판에서 **재확인**되었으나, 거기서 「이미 있는 문의 글을 답변 완료로 표시하는 방법이 없다」로 간 것은 관측이 아니라 **추론**이었고 틀렸다. 같은 리소스의 `POST`가 **`reply_article_no`**(*"If you want to add an reply to a post, enter the number of the post."*)를 받으며 같은 호출에 `reply_status`·`reply_user_id`를 싣는다 — **Cafe24에서는 답변도 글이다**(`parent_article_no`·`reply_sequence`·`reply_depth`). 또 reference에는 **긴급문의 답변**(`GET`/`POST`/`PUT /urgentinquiry/{article_no}/reply`, `content`·`status` F/I/T·`user_id`)이라는 별도 리소스가 있고, 그것이 reference 전체에서 **답변 본문을 발행하는 유일한 리소스**다. 따라서 후보는 A1 답변 글 · A2 댓글 · B 긴급문의 답변 셋이며, board 6이 무엇을 쓰는지는 reference가 말하지 않는다 — READ proof manifest가 승인 대기 중이다(같은 문서 §7). 행위자 값은 A1 `writer`+`client_ip`, A2 `writer`+`password`, B `user_id`로 **경로마다 다르고 셋 다 미보유**이나, `member_id = mall_id`이면 작성자가 **상점명으로 표시된다**는 문서화된 출구가 하나 있다 · 현재 연결 scope `mall.read_community,mall.read_order,mall.read_product`(쓰기 미포함) |
