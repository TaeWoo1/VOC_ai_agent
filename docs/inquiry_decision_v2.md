...
- **규칙 기반 「규격 미확정」 줄은 decision이 GROUNDED여도 유지한다**(§7-A 이후 되돌림). 처음 구현은 모든 need가 FULL이면 그 줄을 뺐는데
  (X6a 과잉 되묻기의 원인), 실제 모델이 규격에 따라 달라지는 답을 FULL로 판정하는 것이 측정됐다 — 그 줄 없이 초안은 「3~4가닥」을 이
  고객의 확정 사실로 쓸 수 있다(2026-08-26 사건의 모양). 과잉 되묻기가 안전한 쪽이다.
...
## 7-A. 실제 모델 검증 (`apr-c8715d20` / `wt-7ac563ca`, 2026-09-19, 소진)

S0·S1 × {Decision v2 + 현재 retrieval(`V2M`), + F5(`V2M_F5`)}, S0 F5는 두 번. decision 호출 340회(plan 340 · judge 339), F5 호출
intent 203 · 임베딩 760 · eligibility 189 — 모두 상한 이하 · 다른 모델 0 · 마켓플레이스 0 · WRITE 0 · clone 쓰기는 S1 판매자 문장 벡터
53행뿐.

| canonical 67 | S0-A | S0-V2O | **S0-V2M** | **S0-V2M_F5** (1 / 2) | S1-A | S1-V2O | **S1-V2M** | **S1-V2M_F5** |
|---|---|---|---|---|---|---|---|---|
| strict safe no-ask precision | 0.400 | 1.000 | 0.455 | 0.545 / 0.545 | 0.364 | 1.000 | 0.462 | 0.400 |
| PARTIAL_LEAK | 3 | 0 | 3 | 3 / 3 | 4 | 0 | 4 | 2 |
| **UNDER_CLARIFY**(되물어야 할 것을 답함) | 0 | 0 | 3 | 2 / 2 | 0 | 0 | 3 | 4 |
| WRONG | 2 | 0 | **0** | **0 / 0** | 2 | 0 | **0** | **0** |
| over-clarification / unnecessary escalation | 1 / 3 | 0 / 0 | 0 / 0 | 0 / 0 | 1 / 4 | 0 / 0 | 0 / 0 | 0 / 1 |
| safe automation coverage | 0.500 | 1.000 | 0.625 | 0.750 / 0.750 | 0.444 | 1.000 | 0.667 | 0.444 |
| retrieval recall | 0.364 | 0.909 | 0.818 | 0.818 / 0.818 | 0.257 | 0.943 | 0.771 | 0.743 |
| precedent recall · 틀린 미리 채움 | 0.300 · 1 | 0.467 · 0 | 0.333 · 2 | 0.600 · 3 / 0.633 · 3 | 0.300 · 1 | 0.467 · 0 | 0.167 · 1 | 0.600 · 4 |

- **집계가 샌 case는 0이다.** 남은 PARTIAL_LEAK·UNDER_CLARIFY는 전부 모델이 **모든 need를 FULL/CONDITIONAL로 선언한** case다 — judge가
  gold보다 너그러웠다. 두 모양: (1) **근거 밖 추론을 FULL로** — R `ffc2cc44`(1.5sq 외경이 지식에 없다) · R `dae8554d` n2(T자 분기) ·
  S T6c(높이는 1·2호만) · S T8a / R `f2ff4a0b`(추가상품 목록의 「곡선엘보캡 N호」에서 호수 대응을 추론); (2) **규격에 따라 달라지는 답을
  FULL로** — R `ae51c7f8` · S T6b · T9a · T9b · X9a(판매자 FAQ가 상품 단위로 쓴 「몇 가닥」).
- **WRONG은 모든 모델 arm에서 0**(A의 분실·해외 배송 오인이 사라졌다), 불필요한 escalation 0–1, 과잉 되묻기 0.
- **precedent scope를 judge가 지키지 못했다**: 틀린 미리 채움 1–4건이 전부 `ORDER_ONLY`/`CASE_ONLY`(주소 변경 처리 · 그 시점 재고 ·
  어제 발송)였다 — 「주문·시점에 묶인 답은 넣지 말라」는 지시만으로는 막히지 않는다.
- planner는 need를 gold보다 잘게 나눴다(질문당 1.62–1.71 vs 1.09, 최대 5) — 안전한 쪽의 차이다. plan 실패 1 · judge 실패 2(340 중)는
  전부 운영 사유 NO_ANSWER_BASIS로 끝났다.
- **안정성**(S0 F5 두 번): need 변화 1.4% · case 변화 3.0%(T6c ↔ T13a).
- **실측 호출**(질문당 2회): plan p50 1.7–1.8s · p95 ≈3.4s · 입력 430 · 출력 ≈100 토큰; judge p50 1.8–2.0s · p95 3.2–3.6s · 입력 1.1k(S0) /
  2.1k(S1, 카탈로그 후보) · 출력 ≈105 토큰 ⇒ 질문당 **≈3.5–3.8s**(§8의 4–7s 추정보다 작다).
- **결론: 구조는 설계대로 동작하지만 성공 기준(PARTIAL_LEAK 0 · precision 1.0)은 실제 모델에서 충족되지 않았다.** 병목은 CoverageJudge의
  충분성 판단이다. production 활성화 권고는 하지 않는다. 이 측정 뒤 코드 변경 하나 — 규격 경고 줄 유지(§6).

## 8. 비용·지연 추정 (측정 아님 — 실측은 §7-A)
