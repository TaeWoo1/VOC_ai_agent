...
### 2.10 종류를 말한 것은 하나를 말한 것이 아니다 — Entity Semantics (2026-08-24 추가)

언급(mention)은 **역할**을 가진다. `role: INSTANCE | CATEGORY`는 계획의 필수 구조이고,
**wire 경계에서 계산**되며(모델이 선언하지 않는다), validator·capability audit·scope gate가 그 한
필드를 함께 읽는다. 범주 어휘 표는 저장소에 **하나**뿐이다.

- **INSTANCE** — 셀러가 염두에 둔 특정 개체. **해결 여부와 무관하게** entity 축을 좁힌다. 해결에
  실패하면 답은 없다 — org 근거로 대신하지 않는다(A1 불변).
- **CATEGORY** — 「답변이 필요한 문의」, 「부정 리뷰」, 「상품별」 같은 종류. **아무것도 좁히지 않고**,
  **resolver에 전달되지 않으며**, 계획에 `PRODUCT_OPS`를 요구하지도 않는다(§2.7이 발동하지 않는다).

**입증 책임은 CATEGORY에 있다.** 모든 토큰이 범주 어휘이고 그중 하나가 핵어일 때만 CATEGORY이며,
모르는 단어가 하나라도 있으면 INSTANCE다. 두 오류가 대칭이 아니기 때문이다 — 범주를 개체로 읽으면 run이
조용해지고, 개체를 범주로 읽으면 **남의 상품 근거로 답한다**. 지시어(이/그/저/해당)는 표에 없고, 따라서
개체다.
