...
| Coupang | ORDER_SUMMARY | API (HMAC) | **VERIFIED** | **Verified live** (2026-08-06, `docs/coupang_final_main_first_connection_order_routine_proof_v1.md`, approval `apr-01212e2da29a` on `main` `59c2e6c`, preflight 9/9): on a pristine database a real WING credential achieved first connection, first `ORDER_SUMMARY` sync, `PREPARING → CONNECTED`, and a same-window **idempotent re-sync**, with zero code modification and read-only GETs only (`returnShippingCenters`, `ordersheets`). **Correction 2026-08-19 — this cell previously read "Auth skeleton only" and that was already false**: `CoupangApiConnector.capabilities()` returns `{ORDER_SUMMARY, INQUIRY}` both `CONFIRMED`, and `CoupangOrdersClient` (657 lines) implements the real v5 `ordersheets` day-paging sweep with `nextToken`, per-status sweep and `shipmentBoxId` dedup. The stale wording survived because the live-proof document was never linked from any canonical doc — the lesson that motivated `docs/evidence/INDEX.md`. **Nothing is promoted by this correction**: 운영 지원 stays ❌ (flag off, schedule off) and the seller-facing label stays 표기하지 않음; that promotion is a product-owner decision. **Blocked risk unchanged:** possible conflict from running two seller-tools simultaneously; parallel provider registration recommended. |
| Coupang | REVIEW | 공식 API 없음 · 공식 export 없음 · seller-owned WING 화면 READ_ONLY만 후보 | **BLOCKED** | **Re-confirmed 2026-08-14 by counting the whole documentation catalogue** (11 categories, no review endpoint) — the finding is "not in the list", not "not found". Operator confirmed WING **has** a review screen, offers **no official export**, and offers sellers **no way to reply** — so this channel is acquisition + VOC/analysis only, never a reply path. Public product-page scraping is **not a candidate**: Coupang's 이용약관 effective **2026-09-03** prohibits bot/spider/scraper collection in terms. Three READ_ONLY sittings (`docs/coupang_review_feasibility_v1.md`) established the structure — each review is its own `<tbody>`, anchored by the `노출상품ID (옵션ID)` column, with a 10-digit per-review identifier that is unique where present but covers **7 of 10** rows, and an 11-digit productId on all ten that is **not** a review key. **TECHNICALLY_POSSIBLE = CONDITIONAL_YES; POLICY = UNCLEAR.** §14 has since been **read in full** (2026-08-14, `docs/coupang_review_policy_gate_v1.md`): its 31 items are all selling conduct and it is **silent on automation** — the exposure is not access but *keeping a copy* (서비스 이용 정책 '시스템 부정 행위' 1)·3), 공통 §14③ 복사·복제·가공, 마켓플레이스 §13②). No clause permits it either. **Development = `PILOT_ALLOWED`, GA = `POLICY_GATED`** (product-owner decision 2026-08-14): a written Coupang answer gates **release**, not building, and the pilot is bounded by data minimization D1–D8 (seller-owned WING only; **no author values anywhere, including dedupe keys**; no raw HTML/DOM/screenshot; no external LLM; no marketplace writes). **D5 lifted 2026-08-14: raw review text IS persisted** — a product decision taken with the 복제·가공 clauses in view, not a legal determination. Product shape: acquisition + VOC + `[쿠팡에서 보기]` locate; never a reply channel. Known limitation: Coupang reviews may be **item-level, not seller-transaction-specific**. **Acquisition + Locate v1 LIVE-PROVEN 2026-08-15, re-proven end to end on the corrected manifest at commit `533cafc2`** (`docs/coupang_review_acquisition_v1.md` §6.6 = proof of record): seated sittings on the operator's own WING 상품평 — backfill 3 pages / 24 rows / **22 stored into a database that began empty**, same-range re-sync **stored=0 skipped=22 with the database unchanged**, and a stored review **located and rung** on the live screen (`matches=1`, twice independently). The re-proof followed nine independent-review fixes, the first of which was an Approval Manifest that described rating-only reviews as skipped while they were being stored. Reader resolves every column from Coupang's own header word and resolves the 구매자 column **so it can be excluded** — 0 author values persisted anywhere. Paging is the **seller's** (0 marketplace clicks/inputs/submissions); completion is READ from the pager (`data-wuic-attrs` carries `active`/`disabled`; prev/next are empty `<a>`s drawn in CSS), and a pager it cannot read stops the walk rather than claiming coverage. Identity is the ingestion spine's content hash: **v2** for a written review, **v3** for a textless one (option id folded in) — 86% of this account's 상품평 were rating-only, and Coupang renders a placeholder sentence there which is **never stored as a body**. Known limitation: two textless reviews of the same OPTION on one day at one rating still merge. Locate matches product+option+date+rating+`review-body-fingerprint/v1`, refusing on 0 or 2+. **Locate UX v1 LIVE-PROVEN 2026-08-15, re-proven on merged main at commit `f357fafe`** (`docs/coupang_review_locate_ux_v1.md` §5.1 = proof of record; the earlier `c334b763` sitting predates the independent-review fixes): two presses on one review, each spending its own binding, `matches=1` both times and **one** row outlined after the second — the ring is replaced, not accumulated: `[쿠팡에서 보기]` now HAS a run behind it — a `REVIEW_LOCATE` intent, its own `locate` carrier, and a single-use backend-minted `locateRef` so nothing that identifies the review crosses the browser or the Action Window wire; the agent resolves it over its own session. Proven on the operator's own screen: `matches=1`, **0 reviews stored** (DB unchanged at 22, 0 sync jobs), bindings spent, 0 marketplace actions. That sitting also caught a defect the offline suite could not: the ring had been an `outline` on the matched `<tr>`, which **Chromium does not paint** — every earlier `highlighted=true`, including §6.5/§6.6's, identified the right row but was invisible. The band moved onto the cells and a real-Chromium PIXEL regression now guards it. Still open: a possibly-truncated body cannot be flagged in the product (`bodyExpandable` stops at the agent), `mediaCount` 0 on every row (no-media vs not-detected undecided), one account only. **Lesson 2026-08-16 (#107): one boolean cannot hold two questions.** The capability badge read the pull connector's `supported` and printed 리뷰 미지원 directly above a panel counting 22 collected 상품평 — both statements true, because the connector answers "can the official API serve this" while the record was filled by the Action Window. The fix is an **additive** axis on the operator-facing overview only, `acquisitionPaths[{method, verificationStatus}]` (COUPANG/REVIEW → `ACTION_WINDOW`/`LIVE_PROVEN`, evidenced by the **acquisition** sitting §6.6 above — 22 stored — not by the locate re-proof, which stored nothing and proves a different claim), with `supported`/`verificationStatus`, the `connector_capabilities` table and schedule gating all unchanged: an acquisition path is not a cadence. Status per registration stays **`❌` for the official API**, which is still true. **Lesson 2026-08-16 (#110–#112): a fact about the marketplace must not live on a connector.** "쿠팡은 판매자 리뷰 API를 제공하지 않는다" sat in `CoupangApiConnector.unsupportedScopes()` alone — and that connector is flagged off by default, so on every default environment the note vanished with it and the acquisition badge stood there unanswered. The counterweight to a capability claim is the half most likely to be dropped, and dropping it fails in the **overclaiming** direction. It now lives in `ChannelApiGapRegistry`, keyed by channel, merged into the overview beside whatever the resolved connector says, and still never inferred from `supported=false`. Same day, the copy stopped saying 지원 on the acquisition axis (§4.1 reserves it for 운영 지원): `상품평 수집 경로 확인됨 · Action Window` + `실계정 검증 완료`, and the schedule row says `자동 수집 미지원` — a cadence judgement, not a channel one. **POLICY remains UNCLEAR; nothing promoted.** |
