...
- **화면의 Teach 호출이 8초에 포기했다** — 서버는 ~17초에 저장·재조사·재초안을 끝냈는데 화면은 실패를 말했고, 다시 누르면 같은
  ...
  고친 뒤 **재증명했다**(`apr-3d8b27e5`, 새 clone): teach POST 1회 · 200 · 14.4초, 새로고침 없이 같은 화면에 「저장됨 · 이 상품」과
  「초안 재작성 완료」, 초안 GROUNDED 근거 2, 지식·Teach 기록 중복 0, 과거 답변 불변, 모델 3 · 마켓플레이스 0. 그 초안은 지식에
  없는 「사진을 보내주시면 확인 후 안내드리겠습니다」를 덧붙였다 — 문의 초안에는 리뷰 lane의 `ReviewClaimGuard` 같은 약속 검사가
  없었다. **§2-B에서 고쳤다.**

### 2-B. Inquiry Claim Guard v1 (2026-09-19) — 근거 없는 약속·요청·수치는 저장되지 않는다

**원인은 둘이었다.** (1) 문의 초안은 모델 출력 뒤에 판매자 금지 표현만 검사했다 — 리뷰 lane의 `ReviewClaimGuard`에 해당하는
것이 없었다. (2) 문의 프롬프트(v10)가 확인되지 않은 것에 대해 **「확인 후 안내하겠다고 쓰세요」라고 지시**하고 있었다 — 리뷰
프롬프트는 같은 유예 문장을 금지한다. 재현된 초안의 끝 문장은 그 지시를 따르다 고객 행동 요청까지 지어낸 것이다.
`ReviewClaimGuard`를 그대로 가져와도 막지 못한다 — 그것은 환불·교환 같은 보상 명사만 본다.

**고친 것 — 둘 다 문의에만.** `reviewSystem()`과 `ReviewClaimGuard`는 바이트 단위로 그대로다(보상 명사 목록은 재사용만).

- `InquiryClaimGuard`: 닫힌 네 모양 — **판매자 후속 약속**(확인 후 안내·연락드리겠·조치하겠) · **고객 행동 요청**(사진·영상·
  주문번호·송장 + 보내·첨부·올려…) · **보상**(`ReviewClaimGuard` 목록) · **수치**(단위나 범위가 붙은 숫자). 각 모양은 금지가 아니라
  **대조**다: 모델에게 보여준 근거가 같은 모양을 쓰면 허용되고, 약속·요청은 판매자가 승인한 문장(unknown-fallback · 필수 표현)도
  허용하며, 수치는 고객이 쓴 숫자와 주문 문장도 허용한다. 판매자가 「사진을 받아 확인한 뒤 안내합니다」라고 쓴 지침은 **그
  자료에 한해** 요청을 허용한다(기존 판매자 지침 테스트가 이 과잉 거절을 잡았다). 목록 번호 「1.」은 수치가 아니다.
- 걸리면 **거절**이지 편집이 아니다: 버전 저장 0, basis는 계산된 그대로, 화면에는 「근거에 없는 약속이나 요청이 들어가 초안을
  저장하지 않았습니다」(판매자 금지 표현 거절과 같은 자리).
- 프롬프트 **v11**: 유예 약속을 쓰라는 세 곳을 「확인되지 않은 것은 쓰지 말고 근거가 있는 부분만 답하세요」로 바꾸고, 근거에 없는
  후속 약속과 고객 행동 요청을 금지하는 줄을 더했다. 「근거가 비어 있으면 그것만으로 답을 만들지 말고」 규칙은 유지(인사·감사까지).

**회귀 테스트는 실제 두 초안이다** — 같은 질문·같은 지식 위에서 `apr-3d8b27e5`가 쓴 초안(사진 요청 + 확인 후 안내)은
`SELLER_PROMISE`·`CUSTOMER_ACTION`으로 거절되고, `apr-7c3e91d4`가 쓴 깨끗한 초안은 통과한다(`InquiryClaimGuardTest`,
`KnowledgeIntelligenceClosureTest`의 composer 경로). 과거 답변 원문은 판매자 데이터라 테스트에 넣지 않았다(두 판정에 영향 없음).
backend 4,402 · 실패 0. 모델 0 · 마켓플레이스 0 · 마이그레이션 0.

**하지 않은 것·남은 것**: 사실 검사기가 아니다 — 숫자도 네 모양도 없는 지어낸 원인·설명은 통과한다. 거절된 초안에 대해 Case
화면은 여전히 「초안 없음」만 보이고 거절 이유를 싣지 않는다(문의 화면은 `unavailableMessage`를 보인다). 새 프롬프트 v11의 실제
모델 출력은 **라이브 미관측**이다.

**실모델 확인(`apr-e41f07c9`, `QA_REPLAY_PROVEN`, 같은 날).** 같은 질문·같은 지식의 GROUNDED 문의 두 건(A: v10이 사진 요청과
「확인 후 안내」를 지어낸 재현 Case, B: v10이 깨끗했던 대조 Case)을 각자의 일회용 clone에서 **초안 모델만 켜고** 한 번씩
재생성했다. 둘 다 v11로 **v2가 저장**됐고(GROUNDED · 근거 2 · 거절 메시지 없음) 약속·자료 요청은 없었다. 초안 모델 2 · 다른 모델
0 · 마켓플레이스 0 · 대상 외 쓰기 0. **거절 경로는 실모델에서 나오지 않았다** — 유도하지 않았고, 그 경로는 실제 v10 초안을 쓰는
회귀 테스트가 계속 증명한다. 가드는 거절할 때 걸린 종류를 로그에 남기지 않는다(거절된 초안은 저장되지 않으므로 사후에 알 방법이
없다) — **보고만.**
...
### 2-C. `asked_signature` — 측정 후 기각 (2026-09-19, 구현 0)

**질문.** Teach로 저장된 지식에는 원래 질문 맥락이 제목 「{주제어} 안내」 한 낱말로만 남는다. 그 질문의 `TopicSignature`
(판매자 말뭉치에 이미 있는 낱말만 · 숫자 제거 · 12개 이하 — Answer Memory·SellerGuidance와 같은 규칙)를 지식 행에 한 칸으로
남기고 검색 텍스트에 제목처럼 붙이면, 표현이 다른 비슷한 질문이 그 지식을 찾는가.

**측정**(로컬 harness · 모델 0 · DB 쓰기 0 · SELECT 전용 연결 · 두 lane을 production 부품 — `KnowledgeRetriever.rank`
(semantic off) · `RetrievalQuery.ofCustomer` 형태 사다리 · 주제 판정 — 로 재조립). 데이터는 Demo Org의 실제 과거 답변 23건을
**수정 없이 Teach한 것으로 본 상한**과, prefill 증명 clone에 실제로 저장된 Teach 2건, 그리고 Answer Memory 측정 때 만든 합성
질문 42개(유사 25 · 다른 리스팅 5 · 무관 12)다.

| | 현재 | + asked_signature |
|---|---|---|
| 유사 질문 적중 (25) | 4 | **6** |
| 무관 질문 오탐 (12) | 0 | **0** |
| 유사 질문에 엉뚱한 Teach가 함께 나옴 | 1 | 1 (같은 건) |
| 원래 질문 적중 (23, sanity) | 12 | 15 |
| 실제 Teach 2건 — 유사 질문 적중 (5) | 1 | 1 |
| 다른 리스팅 질문 (5) | 0 | 0 (리스팅 범위의 문제, signature 무관) |

B가 놓친 유사 질문 19개 중 **18개가 absence 게이트**(`MIN_ASKABLE_RATIO` — 질문 낱말이 판매자 말뭉치 **전체**에 없다:
「떼어내요」·「높낮이」·「입고」)이고 1개가 coverage(0.27 < 0.4)다. 주제 판정·밀려남 0.

**기각 이유 — 구조적이다.** signature의 펜스는 「판매자 말뭉치에 있는 낱말만」이고 absence 게이트는 「질문 낱말이 말뭉치에
있는가」다. 같은 것을 보므로 **펜스를 지킨 signature는 게이트를 뒤집을 수 없고**, 올릴 수 있는 것은 한 지식의 coverage뿐이며
그것도 유사 질문이 원래 질문의 낱말을 다시 쓸 때만이다. 실제 Teach(R1)에서는 판매자가 고친 본문이 이미 그 낱말들을 담아 더하는
것이 0이었고, signature 23개 중 2개는 비고 여럿이 조사·어미(「으로」·「다시」·「사용」·「드립니다 경우」)였다. 펜스를 판매자
확인으로 풀어도 원래 질문(「분리」)과 유사 질문(「떼어내요」)의 낱말이 달라 lexical로는 만나지 않는다(사례에서의 추론, 미측정).
**표현 차이는 lexical 메타데이터가 아니라 의미 lane의 일이다** — 같은 코퍼스에서 임베딩 17/25, F5 14/25 · 오탐 0이 측정돼 있다.
그래서 스키마·Teach 경로·검색 텍스트 **변경 0**. harness는 저장소에 넣지 않았다(기각한 arm이다).

### 2-D. F5 retrieval — Demo Org 한정 측정 (2026-09-19, `apr-8610b0cc`, 채택 결정 없음)

**질문.** 이미 있는 F5(임베딩 + intent 재진술 + 거절 전용 eligibility)를 Demo Org에만 켜면 production
`InquiryKnowledgeAssessor` — 조사와 초안이 같이 읽는 단일 판정 — 에서 과거 답변을 더 찾는가, 무관한 답을 후보로 올리지
않는가, Seller에게 다시 묻는 Case가 줄어드는가. **이 절은 측정 기록이고 production 채택 결론을 내리지 않는다**; 전역 기본값은
OFF 그대로다.

**설정.** 일회용 clone `sellerops_f5_eval` ← `sellerops`(V113로 migrate, dev DB 무접촉). 질문 68 = 합성 S 42 + 실제 R 26
(상품 연결된 실제 문의 31건 중 주문번호·주소·송장번호가 든 5건은 **판정 전에 제외** — 검색에도 벤더에도 들어가지 않았다).
라벨은 arm 실행 전에 고정(`labels.tsv` sha256 `669e1b64…`, 저장소 밖 — 실제 고객 문장이 들어 있다). 판정은 `STORED_ONLY`
(주문·카탈로그 상세 채널 읽기 0), 초안·플래너·조사 모델 OFF, 기동 시 과거 답변 가져오기·리뷰 이슈 재추출 runner OFF,
측정 메서드는 롤백. F5는 **harness 프로세스 env로만** Demo Org 한 곳에 켰다. 64문장 lexical fallback은 세 lane 모두 없음
(최대 과거 답변 17 · 상품 29 · 운영 기준 5).

**Seller-ask 매핑**(고정 라벨에서 파생): 라벨이 맞는 현재 지식 출처를 하나 이상 가지면 `SELLER_NOT_REQUIRED`(18),
아니면 `SELLER_REQUIRED`(50) — 과거 답변만 있는 질문은 REQUIRED다(과거 답변은 근거가 되지 않는다). 양성 = 묻지 않음
(GROUNDED·NEEDS_CLARIFICATION). 묻지 않았는데 근거가 틀리면 라벨과 무관하게 FP.

| | A (F5 OFF) | B 1회차 | B 2회차 |
|---|---|---|---|
| **FP** | 2 | **0** | **0** |
| TP / FN / TN | 8 / 10 / 48 | 13 / 5 / 50 | 14 / 4 / 50 |
| no-ask precision / recall | 0.800 / 0.444 | 1.000 / 0.722 | 1.000 / 0.778 |
| GROUNDED (근거 틀림) | 4 (1) | 6 (0) | 6 (0) |
| NEEDS_CLARIFICATION (근거 틀림) | 6 (1) | 7 (0) | 8 (0) |
| Seller ask (빈 칸 / 미리 채움) | 58 (53 / 5) | 55 (38 / 17) | 54 (40 / 14) |
| 미리 채움 맞음 / 틀림 | 4 / 1 | 15 / 2 | 13 / 1 |
| 과거 답변 lane 정답 (R 8 · S 30) | 3 · 6 | 6 · 15 | 5 · 14 |
| 무관 질문 12에 과거 답변 | 0 | 0 | 0 |

- **FP 2 → 0.** A의 두 FP(배송 후 분실 → 배송 정책 GROUNDED, 해외 배송 → 같은 정책 되묻기)가 F5에서 둘 다 사라졌다.
- **새로 묻지 않게 된 6건은 원문을 읽고 전부 출처가 맞았다.** 다만 GROUNDED 2건은 질문 **일부만** 덮는다(마감캡·엘보 /
  이 주문이 왜 늦었는가는 지식에 없다) — 초안이 덮이지 않은 부분을 지어낼 위험이 남고 Claim Guard는 숫자·약속이 없는 지어낸
  설명을 잡지 못한다. 「몇 호를 사야 하나」에 규격을 되묻는 2건은 `SpecApplicability`의 모양이고 A에도 있었다.
- **남은 부정적 관측.** 무관한 미리 채움 1건이 두 회차 모두 재현됐다(곡선엘보 질문 ← 「10mm 케이블은 3호」 답변). A가 맞게
  미리 채우던 2건이 F5에서 빈 칸으로 돌아갔다(의미 lane이 부재를 판정하고 lexical은 두 번째 표가 없다). 두 회차 사이 68 중
  3행의 판정이 달랐다.
- **Seller ask는 0.853 → 0.809 / 0.794로 소폭**이고, 큰 변화는 빈 칸 → 미리 채움(53 → 38 / 40)이다.

호출(1회차 / 2회차): intent 68 / 68 · 질문 임베딩 135 / 134 · 문단 임베딩 53문장 / 0(캐시) · eligibility 60 / 63 —
