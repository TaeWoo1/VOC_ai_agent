...
- **current blocker:** the fixture-only adapter, boot wiring, readiness evidence, the recordable gates, and the **read-only §8-4 session-precondition probe (EXECUTED 2026-07-12, `ready:true`/`LOGGED_IN`)** are all **complete** — **G1/G2/G3/G4/G5 ✅** ([`decisions.md`](decisions.md) D-021/D-024; [`r4-gate-record.md`](r4-gate-record.md); G3 confirmed 2026-07-12 for the read-only probe path). The one G6 approval used for that probe is **consumed** (per-run, never standing). Still blocking a **live export pilot** (operator/PO-owned, not Runtime code): a **fresh per-run G6 under the full §4 boundary** — the recorded pause lift and consumed G6 were scoped to the read-only probe only.
- **next single action:** record this §8-4 probe-result docs slice on explicit instruction (this task). **The remaining R4 path to a live export pilot is operator/product-owner-owned, not Runtime code:** the read-only probe having verified the session READY/LOGGED_IN, any export pilot needs a **new** per-run G6 approval in that dispatching turn under the **full §4 boundary** (real seller click + download + downstream), plus the environment/live-work-pause posture re-affirmed for an actual export run. Deferred Runtime items: **browser-driver real NAVER-driver ingest** over a real `Download`; a **dedicated `INGEST_FAILED` contract code** (governed contract + FE mapping); the gated real-backend CSV test when a local dev backend is available; FE copy mapping for `actionWindow.step.downstream` / `actionWindow.run.naver`. **Still no live export contact, no marketplace export pages.**
