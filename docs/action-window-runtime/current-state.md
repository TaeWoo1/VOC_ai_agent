...
- **current blocker:** the fixture-only adapter, boot wiring, readiness evidence, the read-only session-precondition probe entrypoint, and the recordable gates are all **complete** — **G1/G2/G4/G5 ✅** ([`decisions.md`](decisions.md) D-021/D-024; [`r4-gate-record.md`](r4-gate-record.md)). Still blocking **live** (operator/PO-owned, not Runtime code): **G3** (stable environment + the NAVER live-work pause lift, [`r4-preparation.md`](r4-preparation.md) §9 item 3) and **G6** (explicit per-run approval in the dispatching turn) — the only two remaining live gates.
- **next single action:** commit this gate-record docs slice on explicit instruction. **The remaining R4 path to a live pilot is operator/product-owner-owned, not Runtime code:** the operator confirms the **G3** stable-environment checklist ([`r4-gate-record.md`](r4-gate-record.md) §G3) and **lifts the NAVER live-work pause** in a stable environment, then gives **G6** per-run approval in the dispatching turn for the **read-only session-precondition probe** — then, and only then, that one permitted live contact runs and its sanitized `{ ready, verdict, blockerCode }` is recorded into [`r4-evidence-pack.md`](r4-evidence-pack.md) §8-4. Deferred Runtime items: **browser-driver real NAVER-driver ingest** over a real `Download`; a **dedicated `INGEST_FAILED` contract code** (governed contract + FE mapping); the gated real-backend CSV test when a local dev backend is available; FE copy mapping for `actionWindow.step.downstream` / `actionWindow.run.naver`. **Still no live contact, no marketplace pages.**
