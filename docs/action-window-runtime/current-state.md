...
- **baseline main SHA:** `3cda125` (`origin/main`; incl. **PR #227 — NAVER fixture Bridge/local-agent boot wiring (D-023)** merged, on top of FE PR #226 (Bridge connection-status UX), PR #225 complete fixture-only NAVER downstream handoff, PR #224 upstream stages, FE PR #223, PR #222 fixture download ladder, PR #221 channel-neutral downstream engine, PR #220 R4-preparation docs, PR #219 R3)
- **current branch:** `feat/r4-supervised-channel-runtime` — HEAD `3cda125` (== `origin/main` after the PR #227 merge fast-forward); pending UNCOMMITTED work: the **§8 pre-live evidence/gate pack slice** (`r4-evidence-pack.md` + a NAVER operator-abort drill) plus this docs sync, to ride together as one commit
...
- **current blocker:** **G1 RESOLVED** ([`decisions.md`](decisions.md) D-021 — NAVER SmartStore review export); the fixture-only adapter, boot wiring, and readiness evidence are all **complete**. Still blocking **live** (all operator/PO-owned, not Runtime code): §3 G2–G6 (incl. pilot seller identity for G2 consent), the NAVER live-work pause lift ([`r4-preparation.md`](r4-preparation.md) §9 item 3), and per-run PO approval in the dispatching turn.
- **next single action:** commit the §8 evidence-pack slice (+ this docs sync) on explicit instruction, then push/PR it as one meaningful slice. **The remaining R4 path to a live pilot is operator/product-owner-owned, not Runtime code:** obtain G2 consent (name the pilot seller), lift the live-work pause in a stable environment (G3), log the §5 policy track (G5), and get G6 per-run approval — then, and only then, a separately-approved read-only live session probe (§8-4). Deferred Runtime items: **browser-driver real NAVER-driver ingest** over a real `Download`; a **dedicated `INGEST_FAILED` contract code** (governed contract + FE mapping); the gated real-backend CSV test when a local dev backend is available; FE copy mapping for `actionWindow.step.downstream` / `actionWindow.run.naver`. **Still no live contact, no marketplace pages.**
...
- **The §8 pre-live evidence/gate pack is ASSEMBLED — UNCOMMITTED (this working tree, on top of merged
  ...
  enums/booleans/counts/test-names/SHAs only) gathers §8 items 1–7: the §3 gate record (G1/G4 ✅;
  **G2/G3/G5/G6 OPEN**, operator/PO-owned), the §6 synthetic ladder (every readiness item green on NAVER
  ...
  path to a live pilot is operator/PO-owned (G2 consent, G3 + live-work pause lift, G5 policy log, G6
  per-run approval), not Runtime code.**
