...
- **baseline main SHA:** `7292217` (`origin/main`; incl. PR #212 R0, PR #214 canonical contract, PR #213 R1, PR #215 FE mock-flow, PR #216 R1.1, PR #217 R2A, PR #218 R2B, and **PR #219 R3** all merged)
- **current branch:** `prep/r4-supervised-channel` (R4 preparation — docs only)
- **current worktree:** `sellerops-r4-prep` (fresh writer worktree; the R3 writer/audit worktrees were removed after the audited merge)
- **branch base SHA:** `7292217` (`origin/main`)
- **shared contract version/path:** **`contracts/action-window/v1/` (`ACTION_WINDOW_PROTOCOL_VERSION = 1`, `ACTION_WINDOW_TRANSPORT_VERSION = 1`) — UNCHANGED (no code in this slice).** Runtime persistence record remains `OPERATION_RUN_SCHEMA_VERSION = 1` (R3, merged).
- **current slice:** **R4 preparation — planning/readiness DOCS ONLY, no live-channel code, no live marketplace contact.** Deliverable: [`r4-preparation.md`](r4-preparation.md) — readiness checklist, first-channel selection criteria (+ evidence scoring), supervised-pilot internal gate, live-action safety boundary, parallel platform-policy/provider-inquiry checklist, technical adapter readiness checklist, rollback/abort criteria, and the pre-live evidence pack. New durable rulings recorded: **D-019** (policy/registration run in parallel; not an indefinite blocker for the supervised user-direct pilot) and **D-020** (first channel is NOT automatically ESM+; five weighted selection criteria).
- **last completed item:** R3 merged (PR #219, merge `7292217`) after an independent clean-worktree audit at `cb47175` — PASS on all persistence/privacy/resume/terminal gates; collector 2443/12 skip, backend 984/0 (offline H2). R3 evidence lives in [`checklist.md`](checklist.md) row 13.
- **last verified tests:** unchanged from R3 (docs-only slice — collector/backend suites not affected; link/path + `git diff --check` + secret-scan validation run on the docs).
- **current blocker:** the pilot channel cannot be selected from repository evidence alone — **§2 criterion 1 (pilot company's actual channel usage) is an unresolved product-owner input** and is decisive. Full ordered list: [`r4-preparation.md`](r4-preparation.md) §9. Current-evidence recommendation (conditional, not a selection): NAVER review export leads criteria 2–5; ESM+ review export strong second.
- **next single action:** product owner resolves [`r4-preparation.md`](r4-preparation.md) §9 items 1–2 (pilot company channel mix → channel ratification G1); no adapter code before the §3 internal gate is signed.
