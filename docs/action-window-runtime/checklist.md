...
| 12 | FE integration (R2) | `IMPLEMENTED` | transport `contracts/action-window/v1/transport.ts`; Runtime `collector/src/action-window/session.ts`, `browser-driver.ts`; FE `frontend/src/lib/actionWindow/bridgeAdapter.ts`, `controller.ts`, `devMode.ts` | `test/action-window/session-integration.test.ts` — 8 (offline loopback E2E); FE `bridgeAdapter.test.ts` — 9 | `test/action-window/session-browser.test.ts` (RUN_INTEGRATION) — **written, NOT run** (no live/headed approval) | `integ/action-window-v1`, PR (open) | FE↔loopback↔Runtime synthetic E2E green offline (start→checkpoint→user-action→recheck→verify→downstream→completed; stale/dup/pause/resume/cancel/reconnect/privacy). Transport rides **inside** Bridge v1 as opaque frames — `bridge/protocol.ts` unchanged. Real Bridge-WS transport still deferred (`resolveBridgeSession()` → null; FE stays on mock). No live channel. |
