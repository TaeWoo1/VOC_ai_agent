...
| 14 | First pilot adapter (R4) | `NOT_STARTED` (preparation docs `IN_PROGRESS` → see 14p) | seams: `export-target-readiness*.ts`, `collector/src/upload.ts` | — | — | — | Channel **not final and not automatically ESM+** (D-020: five weighted criteria; current evidence favors NAVER review export on criteria 2–5, pilot-company channel mix decisive and unresolved). Live requires the `r4-preparation.md` §3 gate incl. per-run PO approval; policy/provider track runs in parallel (D-019). |
