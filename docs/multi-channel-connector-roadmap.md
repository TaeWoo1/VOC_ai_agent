...
- **API 커넥터 상태** (2026-07-07 갱신 — 상세·증거는 §4.1 현행표가 정본):
  ...
  |---|---|---|---|
  | NAVER | `NAVER` | `NaverApiConnector` | ORDER_SUMMARY **구현 + 라이브 1회 검증**(2026-06-14, `docs/archive/sellerops_phase3c_live_smoke.md` §0). REVIEW 공식 API **없음**(코드 주석 확인) |
...
| NAVER | ORDER_SUMMARY | API | ✅ 키 등록 폼 | ✅ | ✅ 2026-06-14, **2026-08-22 재검증** (bounded 최근 14일 창, `backfill` lane, primary cursor 불변; 08-21·08-22 두 날짜에 27건) | ❌ (스케줄 off — 운영자 pause 유지). **routine semantics 2026-08-22 감사·수정**: routine cursor가 14일 넘게 뒤처지면 재개하지 않고 최근 14일에서 재시작(KST 자정 정렬 + emission floor), 14일 이내 지연은 이어서 따라잡음, backfill lane은 불변 — `docs/demo_org_and_channel_knowledge_v1.md` §4d | `docs/archive/sellerops_phase3c_live_smoke.md` §0 · `docs/evidence/INDEX.md` | "자동 수집 지원: 주문" |
| NAVER | REVIEW | EXPORT(감독형) + MANUAL | — (브라우저 세션) | ✅ collector | ✅ 캡처→저장 (2026-06-22); **export→ingest end-to-end 라이브 검증 (2026-07-15, Run 4 — 감독형·개발셀러·로컬 dev 백엔드)**. **2026-08-23 canonical Demo Org refresh — 데이터 `PASS` / 경로 `PARTIAL`**: 리뷰 3,858→4,340(+482), 최신 2026-07-15→2026-08-22, 2구간 COVERED, 중복 0, attribution 100%. 다만 guided flow가 scope MATCH 이후 멈추고(download listener 미무장) manual fallback은 확장자 없는 실제 export 파일을 못 받아, 마지막 두 단계를 운영자가 메웠다. **2026-08-23 completion hardening 완료** — (1) driver fault가 `RUNTIME_FAULT`로 명시적 실패가 되고 패널이 그 실패를 셀러 화면에 계속 표시, (2) download listener가 export barrier에서 무장, (3) 파일 판별이 확장자가 아니라 **바이트** 기준이 되어 자동/수동 경로가 같은 계약 공유. 회귀 green(collector 9,159 · backend 2,679 · frontend 2,237), 새 fence는 수정 전 코드에서 실패 확인. **다음 refresh부터 반복 가능**으로 판정(2026-08-23). 수정 후 마켓플레이스 재실행은 **다음 실제 refresh(2026-09 구간)의 regression checkpoint**이며 현재 blocker가 아니다 | ❌ | `collector/README.md`, collector 트랙 기록, `docs/action-window-runtime/r4-evidence-pack.md` §8-17 | "네이버 리뷰 export 업로드 지원" |
| NAVER | **INQUIRY / 상품 문의 (Q&A)** | API (`GET /external/v1/contents/qnas`) | — | ✅ 구현됨 (`NaverProductQnaClient`, 플래그 `…naver.inquiry.product-qna.enabled`) | ✅ **`CONFIRMED`** — **라이브 1회 (2026-08-24)**, canonical Demo Org, bounded 2026-06-01~08-24, **요청 2회**(토큰 1 + 목록 1, `page 1/1`), **13건 수집 · 오류 0**. **실제 필드 범위**: `questionId`·`createDate`·`question`·`answered` 13/13, `answer` 13/13(전부 답변 완료), `productId` **13/13**. **해결된 계약 질문**: `contents.productId`는 **채널상품번호**다 — `channel_products.external_product_id` 정확 일치 **13/13 = 100%**(원상품번호였다면 0). canonical product 6개 귀속, **신규 product 0**. **비밀글 필드 없음 · 주문 식별자 없음 · raw status token 없음** ⇒ 그 셋은 여전히 주장하지 않는다 | ✅ **`CONFIRMED`** (2026-08-24 20:54, `BLOCKED_EXTERNAL`에서 승격) — **첫 production-like routine run**: 60분 schedule **정확히 1개**, `SCHEDULED SUCCESS 2 received / 0 inserted / 2 skipped / 0 failed`, 두 source가 **한 run 안에서** 각자의 lane으로 실행. 창은 `ROUTINE_MAX_LAG` **정확히 14일**(2026-08-10~08-24) — history를 걷지 않았다. **즉시 재실행(L3)**: `0/0/0/0`, qna 창이 직전 `windowTo`에서 정확히 이어짐(겹침 계약), **중복 canonical row 0 · 커서 단조 전진 · lane 독립**. `primary` 커서가 이때 처음 생겼고 `backfill` 커서는 **한 글자도 움직이지 않았다**. 요청: L2 **3회**(토큰 1 + lane당 목록 1) · L3 **2회**(토큰 캐시). 401/403/429/WARN/ERROR **0** | `docs/naver_inquiry_api_audit_v1.md` §8·§16 · `docs/evidence/INDEX.md` | **source 획득 · routine recurrence 둘 다 증명됨.** 여전히 미증명: 미답변 문의 큐 경로(관측된 18건이 전부 답변 완료) · 2026-06-01 이전 |
| NAVER | **INQUIRY / 고객 문의 (네이버페이)** | API (`GET /external/v1/pay-user/inquiries`) | — | ✅ 구현됨 (`NaverCustomerInquiriesClient`, 플래그 `…naver.inquiry.customer.enabled`) | ✅ **`CONFIRMED`** — **라이브 2회 (2026-08-24)**, canonical Demo Org, bounded 2026-06-01~08-24, 각 **요청 2회**(`page 1/1`), **5건 수집 · 오류 0**, 그리고 **동일 창 재독 `5 received / 0 inserted / 5 skipped`** — 재수집 멱등 실측. **해결된 계약 질문 둘**: (a) 판매자 토큰으로 호출하면 **판매자가 받은 문의**가 내려온다 — 문서는 "구매회원 본인 계정"이라고만 해서 단정할 수 없었으나, 5/5가 이 판매자 자신의 리스팅을 가리키고 5/5에 이 판매자의 답변이 들어 있었다. (b) `content.productNo`도 **채널상품번호**다 — 정확 일치 **5/5**, canonical product 5개, **신규 product 0**. **`customerId`/`customerName`이 실제로 내려왔고 저장 0 · 로그 0**(`author` 5/5 NULL). `answer_body`/`answered_at` 5/5 보존 | ✅ **`CONFIRMED`** (2026-08-24 20:54) — 위 상품 문의 행과 **같은 routine run**. 이 lane도 14일 창에서 `rows=1 page 1/1 last=true` → 0 insert / 1 skip, L3에서 `rows=0`. 두 lane의 커서가 한 문자열 안에서 **각자 전진**하는 것이 라이브로 확인됨 | `docs/naver_inquiry_api_audit_v1.md` §9·§16 · `docs/evidence/INDEX.md` | **source 획득 · routine recurrence 둘 다 증명됨** |
| NAVER | **INQUIRY / TalkTalk** | **공식 API 없음** — 커머스 API 문의 도메인에 TalkTalk endpoint가 존재하지 않음(2026-08-24 vendor 인덱스 전수 확인). 없는 API를 추측하지 않고 이번 범위에서 Action Window도 만들지 않는다 | — | ❌ | ❌ | ❌ | `docs/naver_inquiry_api_audit_v1.md` §0 | **coverage limitation으로 표기** (0건과 같은 뜻이 아니다) |
...
| NAVER | **PRODUCT** | API (`/external/v1/products/search`) | — | ✅ 구현됨 (`NaverProductsClient`, 채널상품 단위) | ✅ **`CONFIRMED`** — **라이브 1회 (2026-08-22)**, canonical Demo Org, 69 리스팅·2페이지·오류 0. **실제 필드 범위**: 이름·판매가·판매상태·통화·최종수정일·카테고리 **69/69**, 브랜드·제조사 **44/69**; **URL 0 · 옵션조합 0 · 상세설명 0 · 판매자관리코드 0** — 목록 리소스가 담지 않는다(상품별 별도 read 필요) | ❌ (스케줄 미생성). **recurrence 2026-08-22 감사·수정**: sweep 종료 시 커서가 1페이지로 복귀 — 이전에는 `3`에 머물러 재관측 불가였다. changed-since 계약 **없음**(요청은 page·size뿐) ⇒ 매 주기 전체 재읽기 + idempotent upsert — 같은 문서 §4d | `docs/evidence/INDEX.md` · `docs/demo_org_and_channel_knowledge_v1.md` §4c·§4d | 표기하지 않음 (판매자 앱에 상품 API 권한 필요) |
| Cafe24 | **ORDER_DETAIL / 단건 조회 (문의 결합 전용)** | API (`GET /api/v2/admin/orders/{order_id}`) | ✅ (동일 OAuth, scope `mall.read_order` **이미 보유**) | ✅ 구현됨 (`Cafe24ExactOrderReader` + `Cafe24OrdersClient.fetchOne`, 커넥터 플래그 뒤 `@Bean`; embed 0 · query 0 · 요청 1건/주문) | ❌ **`LIVE_ORDER_LOOKUP_NOT_RUN`** — REAL 문의 중 `source_order_ref`를 가진 건이 **0건**이라 시험할 대상이 없다. 문의와 무관한 과거 주문번호로 만든 proof는 이 기능이 하는 일(=**문의가 지목한** 주문을 읽는다)을 증명하지 않으므로 만들지 않았다 | ❌ (스케줄 없음 — **스케줄될 수 있는 종류가 아니다**: 사람이 문의 상세를 열거나 초안을 만들 때만 실행되는 bounded read) | `docs/exact_operational_context_v1.md` · 공식 사본 `docs/vendor/cafe24-admin-api/get-orders-order-id.md` | 표기하지 않음 |
| Cafe24 | **PRODUCT** | API (`/api/v2/admin/products`) | — | ✅ 구현됨 (`Cafe24ProductsClient`, offset 페이징) | ✅ **`CONFIRMED`** — **라이브 1회 (2026-08-22)**, canonical Demo Org, 승인된 one-shot READ (run `44bea7b3`, 2페이지 100+44 · **144 리스팅** · 오류 0 · 2.1s), 후속으로 미해결 리뷰 **124/124 relink**. **실제 필드 범위**: 이름·판매가·판매상태·통화 **144/144** — 세 채널 중 상품 판매상태가 실제 값으로 파싱되는 유일한 채널이다(쿠팡은 68/68 `UNKNOWN`); **URL 0 · 옵션조합 0 · 카테고리/브랜드 0** — 목록 리소스가 담지 않는다. **정정 2026-09-08**: 이 칸은 그 라이브 뒤에도 `NEEDS_VERIFICATION`으로 남아 있었고, Canonical Product Truth 초안이 이 칸을 인용해 「실제 응답 모양이 관측된 적이 없습니다」라는 판매자 문장을 만들었다 — stale 정본이 원장을 오염시킨 실제 사례다 | ❌ (스케줄 미생성) | `docs/evidence/INDEX.md` (2026-08-22) | 표기하지 않음 (상품 읽기 권한 없이 만들어진 **오래된 연결만** 재동의 필요 — 현재 요청 스코프에는 `mall.read_product`가 포함된다) |
| Coupang | **PRODUCT** | API (seller-products list + detail) | — | ✅ 구현됨 (`CoupangSellerProductsClient`, 옵션축 `vendorItemId` + 노출상품ID alias 포함; 순회 요청 상한 250 · page 100) | ✅ **`CONFIRMED`** — **2026-08-23 라이브 2회.** ① wire shape 관측(68상품·405옵션·69요청) ② **동일 범위 재수집 멱등 검증**(run `d329ff9e`, SUCCESS 68/68/0/0, requests 69, **신규 리스팅 0 · 신규 product 0** · 옵션 405/distinct 405 · fact 188 불변 · 커서 완주 · WARN/ERROR/401/403/429 **0**). 단서: 이 두 번째 주기는 새 alias 컬럼을 처음 채웠으므로 "아무것도 쓰지 않은" 재수집은 아니다 — 움직인 것은 identity가 아니라 alias이고 행 수는 어느 테이블에서도 불변. **식별자 3개가 이제 3개로 유지된다**: 등록상품ID `sellerProductId`(리스팅 키·SKU) · 옵션ID `vendorItemId`(옵션축) · **노출상품ID `productId`(`external_display_product_id` alias, V60)** — 상품평 화면이 말하는 유일한 상품 식별자이며, 저장 전에는 리스팅 68개 중 **0개**가 매칭됐을 것이다(길이부터 다르다: 노출 8~10자리 vs 등록 11자리). ⚠ **노출상품ID는 1:1이 아니다** — 63 distinct 중 **5개가 등록상품 2개씩**을 앞에 두고 서로 다른 product를 가리킨다(리스팅 10/68). 리뷰 resolve는 그 경우 **fail-closed**. 이전 기록(`NEEDS_VERIFICATION` — 재수집 미검증)은 `docs/demo_org_and_channel_knowledge_v1.md` §5h가 남긴 그대로이고 §5j가 그 바를 넘었다. **wire shape 상세는 2026-08-23 관측됨**(canonical Demo Org 실계정, 68상품·405옵션·69요청, `docs/demo_org_and_channel_knowledge_v1.md` §5h·§5j). 관측이 드러낸 결함 둘(둘 다 backlog, 미수정): 판매상태가 **68/68 `UNKNOWN`**(매퍼가 한글 `statusName`을 영문 enum 사전에 넘긴다 — 상세 응답의 `status`는 존재하나 미독), `manufacture`(68/68 존재·39 채움)와 `contents`(1,196 블록)가 같은 응답에 있는데 미독 | ❌ | 동상 | 표기하지 않음 |
...
> **ORDER_DETAIL 행 신설 (2026-08-25, Exact Operational Context v1).** Cafe24 주문 **단건** 조회를
> 새 행으로 추가했다. **어떤 기존 칸도 옮기지 않았다** — Cafe24 `ORDER_SUMMARY` 행(집계, E2E PASS)은
> 그대로이고, 새 행은 `구현됨 ✅ / 라이브 검증 ❌ / 셀러 표기 없음`이다. 둘은 같은 grant를 쓰지만 같은
> 사실을 말하지 않는다: 집계 routine은 일정대로 SUCCESS하는데 같은 시각 `channel_orders`의 Cafe24
> per-order 행은 **0**이며, 집계가 번 freshness로 개별 주문을 말하면 어느 주문도 지목하지 못하는 숫자가
> 인용 근거가 된다. 2026-08-25 오전에 이 저장소가 「어느 채널에도 exact single-order 계약이 없다」고
> 선언했던 것은 `docs/vendor/`에 대해서는 정확했고 플랫폼에 대해서는 틀렸다 — 정정의 근거는 기억이
> 아니라 사본이며, `ExactOrderLookupCapability`의 선언은 그 파일을 이름으로 가리키고 회귀 테스트가 그
> 파일의 존재를 확인한다.

> **PRODUCT DataType 신설 (2026-08-21, Operator Graph v2).** 세 채널의 상품 카탈로그 read를 정식
> capability로 추가했다. **어떤 기존 칸도 옮기지 않았다** — 새 행 셋은 전부 `구현됨 / 라이브 검증 ❌ /
> NEEDS_VERIFICATION`이고, 셀러 표기는 셋 다 "표기하지 않음"이다. 이유는 두 가지다: (a) 어떤 채널의
> 상품 응답도 이 저장소에서 관측된 적이 없다(ESM INQUIRY 스켈레톤과 같은 상태), (b) Cafe24는 스코프
> 재동의가, NAVER는 애플리케이션 상품 API 권한이 **판매자의 행동으로** 필요하며 우회하지 않았다.
> 마켓 접속 0회로 채울 수 있는 부분(`DERIVED:INGEST` listing, 제목 파싱 spec)은 별개의 파생 경로이며
> capability가 아니다 — `docs/operator_graph_v2_local_integration_proof.md` §2.
