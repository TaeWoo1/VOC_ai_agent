...
| NAVER | **PRODUCT** | API (`/external/v1/products/search`) | — | ✅ 구현됨 (`NaverProductsClient`, 채널상품 단위: 이름·URL·가격·판매상태·브랜드/제조사·카테고리·옵션조합) | ❌ **`NEEDS_VERIFICATION`** — wire shape 미관측 | ❌ | 코드 + `ProductKnowledgeCoverageTest` | 표기하지 않음 (판매자 앱에 상품 API 권한 필요) |
