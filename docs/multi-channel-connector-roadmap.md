...
| NAVER | **INQUIRY / 상품 문의 (Q&A)** | API (`GET /external/v1/contents/qnas`) | — | ✅ 구현됨 (`NaverProductQnaClient`, 플래그 `…naver.inquiry.product-qna.enabled`, 기본 off) | **`NEEDS_VERIFICATION`** — 라이브 0회. 공식 계약은 vendor 사본으로 고정(`docs/vendor/naver-commerce-api/get-v1-contents-qnas.md`): `fromDate`/`toDate` 필수, page 1-based, size ≤100, `questionId`·`createDate`·`question`·`answer`·`answered`·`productId`·`productName`·`maskedWriterId`, 종료는 `last`/`totalPages`. **비밀글 필드 없음 · 주문 식별자 없음 · raw status token 없음** ⇒ 그 셋은 주장하지 않는다 | ❌ (자동 schedule 불가 — reconciler는 `CONFIRMED`에만 만든다) | `docs/naver_inquiry_api_audit_v1.md` §1 | 표기하지 않음 (라이브 증명 전) |
