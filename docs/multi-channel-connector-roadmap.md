...
| NAVER | ORDER_SUMMARY | API | ✅ 키 등록 폼 | ✅ | ✅ 2026-06-14, **2026-08-22 재검증** (bounded 최근 14일 창, `backfill` lane, primary cursor 불변; 08-21·08-22 두 날짜에 27건) | ❌ (스케줄 off — 운영자 pause 유지). **routine semantics 2026-08-22 감사·수정**: routine cursor가 14일 넘게 뒤처지면 재개하지 않고 최근 14일에서 재시작(KST 자정 정렬 + emission floor), 14일 이내 지연은 이어서 따라잡음, backfill lane은 불변 — `docs/demo_org_and_channel_knowledge_v1.md` §4d | `docs/sellerops_phase3c_live_smoke.md` §0 · `docs/evidence/INDEX.md` | "자동 수집 지원: 주문" |
...
| Cafe24 | **PRODUCT** | API (`/api/v2/admin/products`) | — | ✅ 구현됨 (`Cafe24ProductsClient`, offset 페이징) | ❌ **`NEEDS_VERIFICATION`** — wire shape 미관측 | ❌ | 동상 | 표기하지 않음 (**`mall.read_product` 재동의 필요** — 기존 연결은 `mall.read_community,mall.read_order`만 보유) |
