...
| Coupang | **PRODUCT** | API (seller-products list + detail) | — | ✅ 구현됨 (`CoupangSellerProductsClient`, 옵션축 `vendorItemId` 포함; 순회 요청 상한 250 · page 100) | ⚠ **`NEEDS_VERIFICATION`** — **wire shape는 2026-08-23 관측됨**(canonical Demo Org 실계정, 68상품·405옵션·69요청, `docs/demo_org_and_channel_knowledge_v1.md` §5h). 이 칸을 `CONFIRMED`로 올리지 **않는** 이유는 하나뿐이다: **동일 범위 재수집(멱등) 미검증.** INQUIRY를 CONFIRMED로 올린 기준(2026-08-14 — 재수집 stored=0/skipped=전건)이 아직 충족되지 않았고, 이번 proof는 승인 범위가 one-shot이었다. 관측이 드러낸 결함 둘: 판매상태가 **68/68 `UNKNOWN`**(매퍼가 한글 `statusName`을 영문 enum 사전에 넘긴다 — 상세 응답의 `status`는 존재하나 미독), `manufacture`(68/68 존재·39 채움)와 `contents`(1,196 블록)가 같은 응답에 있는데 미독 | ❌ | 동상 | 표기하지 않음 |
