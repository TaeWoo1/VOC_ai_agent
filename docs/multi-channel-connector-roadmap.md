...
| NAVER | **INQUIRY / 고객 문의 (네이버페이)** | API (`GET /external/v1/pay-user/inquiries`) | — | ✅ 구현됨 (`NaverCustomerInquiriesClient`, 플래그 `…naver.inquiry.customer.enabled`, 기본 off) | **`NEEDS_VERIFICATION`** — 라이브 0회. 공식 계약(`get-v1-pay-user-inquiries.md`): `startSearchDate`/`endSearchDate` 필수 `yyyy-MM-dd`, page 1~1000000, size 10~200, `inquiryNo`·`title`·`inquiryContent`·`inquiryRegistrationDateTime`·`answered`·`answerContent`·`answerRegistrationDateTime`·`category`·`orderId`·`productNo`. **`customerId`/`customerName`이 필수 응답 필드다 — 읽지 않고 저장하지 않는다.** 판매자 토큰으로 판매자 수신 문의가 내려오는지는 문서만으로 단정 불가 ⇒ 라이브 1회가 답한다 | ❌ (같은 이유) | `docs/naver_inquiry_api_audit_v1.md` §2 | 표기하지 않음 (라이브 증명 전) |
