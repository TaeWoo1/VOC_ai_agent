...
answers). Marketplace 0 · CDN fetch 1 (F, approved) · model calls 10 (investigation 7, draft 2, vision 1).
...
| F | 5★ with 1 photo, «…불량이면 말씀 드리겠습니다» | **QA_REPLAY_PROVEN** — the words send it to investigation. With vision off (first pass) the photo is recorded «보지 못한 사진». With vision on (approved «approve F vision», replayed again as `9000000003`): 1 CDN fetch (`phinf.pstatic.net`) → 1 vision call (`gpt-5.6-terra`) → `INSPECTED`, problem visible `NO`, «투명 비닐 포장 안에 흰색 디스펜서 본체와 설명서로 보이는 종이가 보입니다» → the investigation lists «고객이 올린 사진 1» → Agent `AUTO_RESOLVED` / `NO_ACTION` («5점 리뷰로 사용 전이라고만 하셨습니다») |
