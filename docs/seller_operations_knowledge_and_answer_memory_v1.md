...
## 14. Retrieval precedence — **Answer Memory ≠ Good Answer** (2026-08-26, product-owner)

이 문서가 세운 3-lane retrieval은 「어느 lane도 lane이라는 이유로 이기지 않는다」였고, 그 규칙은
**옳고 유지된다** — 상품 지식과 운영 정책은 여전히 정확히 동등한 peer다. 바뀐 것은 lane 사이의
순위가 아니라, **셋 중 하나가 다른 종류의 주장을 한다는 인식**이다.

> **`EXECUTOR_SENT_VERIFIED`의 의미는 「실제 이 문장이 marketplace에 발송됐다」이지
> 「이 문장이 정답이다」가 아니다.**

과거에 실제로 발송된 답변이라고 해서 그것이 사실적으로 정확하거나, 좋은 고객응대이거나, 앞으로
따라야 할 스타일인 것은 아니다. 그 문장은 **지금은 없는 정책 아래에서**, **그 뒤로 스펙이 바뀐
상품에 대해**, 혹은 그냥 급하게 쓰였을 수 있다. 나머지 lane은 **지금 무엇이 참인가**를 말하고,
틀리면 고쳐지는 출처를 갖는다.

### 14.1 어휘 — `KnowledgeScope.current()`

`PAST_ANSWER`만 `current() == false`다. CURRENT / HISTORICAL 구분은 **관련성**의 문제가 아니라
**주장의 종류**의 문제이므로, 새 enum이나 새 랭킹 엔진이 아니라 기존 scope에 술어 하나로 들어간다.

### 14.2 구조적 우선순위 — 세 줄

`InquiryEvidenceRetriever.merge`의 최소 수정:

1. **reserved slot은 CURRENT lane에만.** 과거 답변은 예약석을 갖지 않는다.
2. **historical은 최대 1개, 그리고 마지막.** 현재 근거를 **밀어낼 수 없고**, 창이 꽉 차면 가장 먼저
   버려진다 — 네 개의 현재 근거가 이미 질문에 답하고 있다면, 무언가가 한때 발송됐다는 것을 증명하는
   다섯 번째 출처는 초안에 필요하지 않다.
3. **과거 답변 *혼자서는* `GROUNDED`가 되지 않는다.** 이것이 실제로 중요한 부분이다. 이전에는
   memory만 매칭돼도 판매자가 **「판매자가 등록한 과거 답변을 근거로 썼습니다」**를 읽었다 —
   그 안의 어떤 사실에도 현재 출처가 없는 초안 위에서. 이제 `GROUNDED`는 CURRENT 근거로만 얻는다.

그리고 판매자가 읽는 문장도 함께 정직해졌다. 비-`GROUNDED` 기본 문구는 「과거 답변에도 해당 내용이
없어」로 끝나는데, memory만 매칭된 경우 그 문장은 **거짓**이고 바로 아래에 그 과거 답변이 인쇄된다.
그 경우에는 「현재 상품 지식·운영 정책에는 이 질문에 해당하는 내용이 없습니다. 아래 과거 답변은
참고용이며, 그때의 안내가 지금도 맞는지 확인이 필요합니다.」로 바뀐다.

### 14.3 무엇이 아닌가

- **2026-08-24가 닫은 lane 랭킹이 아니다.** 그것은 「상품이 정책을 이긴다」를 미리 정하는 것이었고,
  그러면 상품 라이브러리가 얇을 때 「폭이 몇 mm인가요?」가 배송 정책으로 답해진다. 이번 것은 그 둘을
  **동등하게 두고**, 「무엇이 참인가」와 「무엇을 말했었나」를 가른다.
- **과거 답변을 억제하지 않는다.** 여전히 검색되고, 여전히 보이고, 여전히 말투를 맞추는 데 옳은
  참고물이다. 다만 **현재 사실에 대한 권위이기를 멈춘다.**
- **prompt instruction에만 맡기지 않는다.** v6 프롬프트에는 이미 「[과거 답변]은 … 표현을 맞추는 데
  쓰고 사실의 출처로 쓰지 마세요」가 있었다. 그것은 모델에 대한 지시이지 게이트가 아니었고, merge는
  그 지시와 무관하게 memory에 예약석을 주고 있었다.

**DB 변경 0 · 마켓플레이스 호출 0 · 새 테이블/enum/랭킹 엔진 0.**
